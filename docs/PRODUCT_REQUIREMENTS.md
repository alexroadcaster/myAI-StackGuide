...
| R11 | Pin the paired card-2.0.0, policy-2.0.0, index-format-2, catalog and artifact hashes; reproduce candidate selection and fail closed on mixed v1/v2 sets. Remote overlay/compaction remains deferred. | CP-03, CP-06, CP-09, CP-11, CP-13, CP-16 |
...
SQLite FTS5/BM25 is selected. The local `catalog_only` source uses `sqlite_fts5`; no vectors/embedding models/database service required. FTS v2 keeps current name, historical full-name aliases, upstream description and catalog description in distinct columns, alongside topics, category labels, use cases, integration surface and best-for. Retrieve at most 60 candidates across query variants, deliver at most 12 cards and 48 KiB UTF-8 evidence including provenance; these uncalibrated ceilings and initial weights are not token or quality promises. CP-03 allocates Brief and retrieval budgets; CP-04 validates v2 compatibility, RU/EN/aliases, retrieval relevance and integration usefulness. Missing or mixed-version index/FTS5 is an explicit failure, not coercion, rebuild or silent full-catalog context loading.
