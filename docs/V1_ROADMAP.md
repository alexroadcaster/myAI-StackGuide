...
- CAT-08/09/10 are complete at their distinct evidence ceilings. CAT-08 reconciled the 2,500-identity candidate and old-card/addition field map, CAT-09 verified all identities/routes plus representative offline browser behavior, and CAT-10 froze the exact source/output pins and migration handoff. CP-06 now consumes only that frozen version.
- CP-06 must consume the exact CAT-10 snapshot after v2 contract acceptance, emit only self-contained card-2.0.0 records, preserve numeric identity/source lineage/historical names/dual descriptions/typed assignments/unknowns and build the paired public-only policy-2.0.0/index-format-2 assets. Public card facts may not be split into a metadata sidecar. Do not wait for exhaustive recommendation narratives before a useful slice.
