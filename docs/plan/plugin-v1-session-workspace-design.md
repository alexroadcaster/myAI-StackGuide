...
5. Retrieval disclosure: structured intent/aliases/constraints, matched fields, source/index/policy identity, actual candidate/card/byte counts, dedupe/exclusion/truncation reasons. The 150 fetched hits / 12 cards / 160 KiB values are labeled ceilings; a narrow query may request fewer candidates. Ceilings are never reported as actual results by default. Display is not limited to three cards by the sample mockup.
...
The design set comprises eight separate desktop views, one English comparison view; no mobile concepts. Owner visual amendment (2026-09-23): replace the light/forest palette with an Apple-inspired luminous glass direction: silver-white surfaces, soft ice-blue ambient gradients, translucent navigation/header chrome, rounded segmented language controls and restrained highlights. This is a native-CSS web approximation, not an Apple platform material. Content surfaces remain near-opaque for legibility; solid-fill, reduced-transparency/motion, high-contrast and print fallbacks preserve access. No external fonts, images, libraries or animation loop are introduced. The common shell and snapshot/Codex boundary remain; semantic depth increases in B-D. Each synthetic example is labeled as demonstration data. No real repository or scan result is fabricated for visual density. This amendment records design intent, not browser acceptance.
