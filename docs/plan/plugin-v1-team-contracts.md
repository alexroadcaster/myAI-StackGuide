...
- Local C1-C6/C9 specs include the CP-03 presentation/publication addendum and linked historical/current fixtures. The owner-approved CP-03/06 prerequisite expansion and CP-09 implementation establish the active format-3 tuple and verified local read-only route; the earlier format-2 CP-03/C8 results remain historical. Project-local development dependencies now run `SchemaContracts` 48/48 and C8 33/33 plus both synthetic scorer CLI gates. CP-09 is `implemented_verified_local`; CP-04 relevance calibration remains open because the scorer verdict is still `synthetic_compatibility_only`, not because validation is unavailable. Existing CP-02 ADRs are design evidence. Remote C4/C7 belongs to deferred CP-12 and does not block local CP-03. Never claim to have read a proposed schema.
...
- CP-08 never writes canonical state directly. Its frozen local handoff uses exact `commit-scan` body `{report, checkpoint}` and exact `commit-context` body `{selection, brief}`; expected run/revision remain separate CP-07 command arguments. CP-07 validates both bodies under its single-writer lock, resolves selected references against the revision/root/policy-bound private checkpoint ledger, removes the checkpoint on context commit/finalization, and rejects extra body fields.
- `.scan-checkpoint.json` and `.scan-checkpoint.pending.json` are private plugin-owned restart slots under `docs/myai-stackguide/`, governed by `specs/scanner/scanner-restart-checkpoint.schema.json`. They are noncanonical, revision-bound, capped at 2 MiB, counted within the unchanged root caps and never sent to HTML, public catalog/index, retrieval or model context. Do not persist excerpts, raw source, absolute paths, arbitrary raw graph or excluded/sensitive path names. The pending file is never restart truth and can be reused only after plugin-ownership validation under the writer lock. Owner/run/root-fingerprint/policy/hash/revision mismatch or an incompatible phase makes the current checkpoint fail closed; never merge sidecar bytes into canonical state. Cross-file crash recovery may require a bounded rescan and does not imply atomic state/checkpoint publication.
...
CP-06 originally built the paired FTS v2/card-2.0.0 assets; its index-format-2 result is historical. The owner-approved CP-09 prerequisite expansion migrated the active tuple to query/result/evidence/manifest/memo `2.1.0`, request `1.1.0`, card/activity `2.0.0`, policy `2.1.0`, index format `3` and route registry `1.0.0`. CP-09 opens only that paired index read-only, validates the full manifest tuple, compiles/escapes structured queries, applies RRF and request-selected caps, returns numeric canonical IDs and exact matched fields, and checks every leaf/container/review route plus assignment dedupe. Mixed pins and unavailable/corrupt inputs fail explicitly; there is no coercion, rebuild or active v1 fallback. Model context never receives the whole catalog; public index never receives user context.

CP-09 is `implemented_verified_local`: retrieval/matching passed 23/23, index/intake passed 18/18, CP-03 passed 48/48, C8 passed 33/33, both scorer CLI gates exited zero and the builder `--check` verified 2,500 rows/cards, 2,630 assignments, 126 routes/162 memberships, immutable access and snapshot/full-manifest provenance. Independent review found no remaining S1 runtime issue. The scorer remains `synthetic_compatibility_only` with `promotion_ready=false`; relevance, human usefulness, RU/EN or browser behavior, CP-11 integration and release remain later evidence. The unobserved mixed primary-before-reference interaction is a non-blocking residual gap. No remote, MCP, vector or deployment path is activated.
