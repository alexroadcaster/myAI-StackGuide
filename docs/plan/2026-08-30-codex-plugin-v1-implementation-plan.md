...
Current catalog source is `data/catalog_manifest.json`, schema `data/catalog_manifest.schema.json`, shell `templates/unified_catalog.html`; `docs/UNIFIED_CATALOG.html` is generated. The canonical intermediate snapshot is dated 2026-08-31 and contains 1,800 repository records, 126 taxonomy nodes and 1,945 direct placements. CAT-07A separately freezes a non-canonical 2,500-identity candidate over the same taxonomy; CAT-08/09/10 still own canonical reconciliation, verification and handoff. Existing browser enrichment is not reliably persisted source evidence; CP-06 must consume the CAT-10 snapshot and close remaining input gaps before indexing claims about activity. Unknown facts must not be invented.
...
- Current addendum evidence: 22 local schemas; all 46 CP-03 and 27 C8 checks pass, as do both scorer CLI gates. Independent reviewer repeated both full suites without skips or residual P1/P2 in this contract slice. The Completion report below is preserved first-pass history; the Current Addendum Evidence Report owns current acceptance.
...
- Acceptance criteria: Consume only the exact CAT-10 snapshot and its taxonomy/schema/field migration map. Do not rely on browser localStorage/fetched objects as persisted metadata. Keep created/push/verified commit/release/observed dates separate with per-field sources; no stamp-all-current after partial refresh. Normalize old and CAT-07A card shapes without inventing missing legacy fields; preserve numeric GitHub identity, canonical aliases, category kind/parent and baseline-versus-expansion provenance. Map all 2,500 cards to retrieval or explicit rejection, but require mandatory evidence only for primary adoption claims. Test dedupe, sparse cards, index integrity/source parity/pins and one useful search. No user context in bundle; no API/embedding/model download. Actual 2,500-card size and synthetic 10,000-row headroom remain distinct.
...
- Acceptance criteria: Parameterize SQL and escape FTS grammar; handle malformed/empty/no-hit queries; stable ordering and total 60-candidate/12-card/48-KiB ceilings across query variants. Verify all 111 leaf filters, all 14 container descendant unions, the review queue, alias/rename lookup and secondary-placement dedupe against the exact 2,500-card pins. Constraint unknowns produce caveats/next verification, no unconditional fit; snapshot age alone never rejects. Missing FTS5/corrupt/incompatible index returns explicit unavailable/incompatible outcome, never no-match or full-catalog fallback. No writes to index, embeddings, remote calls or hidden query logging.
...
- Acceptance criteria: Use CP-04 frozen cases/runner/judgments and actual CP-11 FTS5 route bound to the 2,500-card snapshot. Inspect RU/EN/aliases, all container semantics through deterministic checks, representative thin/dense leaves, baseline/expansion cohorts, secondary-category dedupe, mature-old versus active-incompatible cases, missing facts/no-match, integration prerequisites and rollback. Avoid a random corpus dominated by the largest expansion leaves. Permit useful ordinary source reads and proposed implementation steps; reject unauthorized execution/disclosure and unsupported operability claims. No CP-12/13/14 dependency. Record model/provider authority and evidence separately if a model run is needed.
...
- Acceptance criteria: Actual supported Windows/Python/FTS5 preflight, installed/cache identity, read-only query, no network/download, state compatibility and previous-package/index rollback verified in an authorized synthetic workspace. Record package, 2,500-card snapshot and index sizes/hashes; verify upgrade from the prior 1,800-card/126-node canonical snapshot to the 2,500-card/126-node snapshot and rollback to the paired prior package/index without silently accepting mismatched state pins. No private artifacts or secrets in package. Owner accepts local scope/evidence; publication needs exact separate authorization and is not required to label local package-ready.
