...
Owner revision: 2026-09-01, with CP-07 and CP-08 local closure accepted 2026-09-21. SQLite FTS5/BM25 is selected for local catalog retrieval. The product optimizes time to a useful open-source integration or modernization plan. Relevant project reads are allowed within existing permissions; the former host-wide source-isolation requirement is superseded, not technically proven. Repository activity and evidence observation are separate; the former 30-day snapshot rejection is withdrawn. CP-01/02 remain completed documentation records. CP-03/C8 contract results remain historical evidence. The owner accepted `RepositoryCardV2` as the sole active public-card representation; CP-06 generated and build-verified exactly 2,500 v2 cards plus the paired immutable SQLite FTS5 v2 index, manifest and policy. CP-05-C is owner-accepted at `measured_local`. CP-07 is implemented at its bounded local writer/intake/preflight ceiling. CP-08 is `completed_local_v13`: bounded scan/context, Brief/corrections, sole-writer commits, private checkpoint recovery, four-root measurements, 100,000-record fresh-process restore and task-specific selection have current local evidence. `policy_limits_calibrated=false` remains because no universal performance SLA or exhaustive arbitrary-project calibration is claimed. Persisted crash injection, joined behavior, human acceptance, install and release remain CP-11/15/16. CP-12-CP-14 remain deferred. Actual CP-09 retrieval routing/relevance, CP-10 UI, joined CP-11 behavior, install, release and publication remain unverified. Permissions are unchanged.
...
| R07 | Positive numeric GitHub repository ID is canonical across card/index/result/evidence/memo/integration joins; catalog IDs are lineage inside card identity and historical full-name aliases are separate. Upstream/catalog descriptions and creation/update/push/verified commit/release/sync/observation dates remain distinct. | CP-03, CP-06, CP-09, CP-10, CP-11 |
| R08 | One public card separates exact catalog status, evidence stage, public provenance, advisory eligibility/completeness and request-specific role; curator-only acceptance remains unchanged. Automatic public candidate overlay is deferred. | CP-03, CP-06, CP-09, CP-12, CP-13 |
...
| R11 | Pin the paired card-2.0.0, policy-2.1.0, index-format-2, catalog and artifact hashes; reproduce candidate selection and fail closed on mixed v1/v2 sets. Remote overlay/compaction remains deferred. | CP-03, CP-06, CP-09, CP-11, CP-13, CP-16 |
...
The CAT-10 canonical source snapshot is frozen at the exact 2026-09-01 manifest hash with 2,500 repositories, 126 taxonomy nodes and 2,630 direct placements. CAT-08 applied the frozen CAT-07A candidate; CAT-09 passed exhaustive source/output parity plus representative offline Chrome startup/search/filter/navigation thresholds for the same projection. The [CAT-10 report](reports/catalog-final-freeze-2026-09-01.json) pins the source/schema/taxonomy/field contract, output hashes and CP-06 migration rules. These counts are snapshot inventory, not live GitHub facts. After the atomic `CP-03-C9-V2` migration, CP-06 created exactly one self-contained v2 card per numeric identity and one immutable derived FTS v2 row. Each card contains exact source lineage, historical names, both descriptions, typed assignments, normalized public facts/advisory/activity and bounded provenance/evidence. Browser-only observations are not reproducible source facts; missing fields remain explicit null/empty values. No public-fact metadata sidecar or active v1 fallback is permitted.

SQLite FTS5/BM25 is selected. The local `catalog_only` source uses `sqlite_fts5`; no vectors/embedding models/database service required. FTS v2 keeps current name, historical full-name aliases, upstream description and catalog description in distinct columns, alongside topics, category labels, use cases, integration surface and best-for. Retrieve with a request-selected budget of at most 150 fetched hits across query variants, deliver at most 12 cards and 160 KiB UTF-8 evidence including provenance within a 200-KiB controlled input; these owner-accepted but uncalibrated ceilings and initial weights are not token, relevance or latency promises. CP-03 allocates Brief and retrieval budgets; CP-04 validates v2 compatibility, RU/EN/aliases, retrieval relevance and integration usefulness. Missing or mixed-version index/FTS5 is an explicit failure, not coercion, rebuild or silent full-catalog context loading.
