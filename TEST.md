...
| CP-03-C9-V2 / R06-R08, R11 | Atomic active-contract migration to one self-contained `RepositoryCardV2`, ActivityV2, retrieval policy 2.1.0 and index format 2; all linked query/result/evidence-pack/manifest fixtures use compatible pins; current 48 contract/workspace and 33 C8 checks pass | Earlier policy-2.0.0 evidence remains historical; mixed v1/v2 refs, generic `repo_id`, collapsed descriptions/statuses, split public-fact sidecar and active v1 fallback rejected |
...
| CP-09 / R06, R07, R11 | Actual read-only FTS5 v2 over the exact 2,500-card pins, compiled/escaped queries, numeric IDs, RU/EN/current names/historical aliases, separate upstream/catalog description matches, all leaf/container/review routes and deterministic assignment dedupe/constraints/pack | Empty/no-hit vs unavailable distinguished; v1/mixed/missing/corrupt index explicit; no coercion, rebuild, whole-catalog fallback, duplicate container hits or index writes |
| CP-09/11 / context limits | Across all variants <=150 fetched hits, <=12 detailed cards, <=160 KiB serialized evidence including provenance and <=200 KiB controlled input; a query may request less | No counter reset across broadening, truncated data passed as complete, or Brief allocation omitted; limits encoded by CP-03 policy 2.1.0 |
...
| CP-11 / R12 | Named useful 1/1 path with actual source_mode=catalog_only, retrieval_engine=sqlite_fts5 and paired card-2.0.0/policy-2.1.0/index-format-2 pins; prove one historical-alias and one catalog-description route; actual 2,500-card capacity plus separately labeled 10,000-row synthetic headroom | No mock/alternate/v1 route masks acceptance; synthetic headroom cannot prove search relevance |
...
`CP-03-C9-V2` updated the active card/activity/query/result/evidence-pack/manifest/policy schemas plus linked fixtures and repeated the C8 compatibility join. CP-06 added and passed `tests/test_plugin_catalog.py` and `tests/test_plugin_search_index.py`; CP-09 next adds `tests/test_plugin_retrieval.py` alongside matching tests. Exact remaining future files/commands are registered in [CP-02 verification](specs/decisions/plugin-v1-verification.md) and the detailed CP tasks. CP-04 owns `evals/plugin-v1/evaluate_retrieval.py`, `tests/test_plugin_retrieval_eval.py` and `tests/fixtures/plugin_retrieval_eval.json`: current results prove offline v2 synthetic compatibility only. Actual retrieval captures, relevance judgments and calibration remain CP-04 work; this is not a runtime retrieval/model runner. Require nonzero expected test counts; unittest discovery with zero tests is not acceptance.
