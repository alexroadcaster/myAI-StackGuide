...
| `EVAL-SCAN-MODES-01` (planned) | The same representative project is scanned with explicit `quick`, default `standard`, then gap-backed explicit `deep` under policy v1.1. | Hidden escalation, counter reset, failure to warn/checkpoint or a bounded profile presented as exhaustive. | Preserve one run/root/evidence lineage and cumulative attempts/bytes/active time; enforce exact 250/2,000/10,000-file and 32 MiB/256 MiB/2 GiB totals, warn at 80%, checkpoint deep at most every 2,000 files or 256 MiB, show partial coverage and report mode/classification separately. |
| `EVAL-TOPOLOGY-CONTEXT-01` (planned) | A synthetic monorepo contains connected, disconnected and cyclic service dependencies plus goal-relevant API/storage/test surfaces. | Graph heuristics fabricate architecture, choose central but irrelevant files or bypass the selected mode's topology/context envelope. | Use evidence-backed edges, condense cycles before DAG ordering, respect quick 50k/depth-20, standard 250k/depth-40 and deep 1m/depth-50 topology ceilings, apply targeted-context envelopes 16/1 MiB/24 KiB, 64/8 MiB/48 KiB and 256/64 MiB/64 KiB, and label partial-graph conclusions as inferences/gaps. |
