...
CP-03.CAT-10 is `catalog_snapshot_frozen_cp06_handoff_ready`: the source-owned snapshot has 2,500 unique repositories, 126 taxonomy nodes and 2,630 placements, with exact source/schema/taxonomy/field-contract/output pins in its final report. It preserves CAT-09 `browser_performance_verified` evidence for the same HTML projection; three review records moved to evidence-supported existing leaves and 12 remain explicit review records. `CP-03-C9-V2` contract-verifies the accepted `RepositoryCardV2`, ActivityV2, retrieval policy 2.1.0, index format 2 and four matching C8 captures. CP-06 proves lossless 2,500-card generation, logical-row parity, immutable SQLite bytes/integrity, paired hashes and build-time alias/catalog-description smoke. CP-07 supplies an owner-accepted bounded writer/intake/preflight interface but no recommendation-quality result; CP-08 supplies completed-local scanner/context evidence under its stated ceiling. The scorer still reports `retrieval_score_v2`, `passed=true`, `promotion_ready=false`, `quality_thresholds_calibrated=false` and `verdict=synthetic_compatibility_only`. Historical v1 results remain dated evidence. CP-06/07/08 do not prove CP-09 actual runtime routing, held-out relevance, comparison usability, human usefulness, visual/privacy acceptance, joined CP-11 behavior, live freshness, install or release readiness; those remain CP-04/09/10/11/15/16 scope.
...
CP-04 first revalidates its captured-result protocol against one paired `RepositoryCardV2`/policy-2.1.0/index-format-2 fixture set, then freezes a simple lexical/filter baseline over those same cards, held-out relevance judgments and query set before comparing the selected FTS5/BM25 implementation. No embedding, vector store or model/provider call is necessary to measure retrieval. A later semantic extension must demonstrate gain over this baseline and account for installation, latency, memory and maintenance costs; it is not a current gate.
...
The [C8 runner contract](evals/plugin-v1/runner-contract.md), scenario/result schemas and scorer are present. The active policy-2.1.0 fixture join covers allowed, no-match, denied and missing-index outcomes. Current verification passes 48 CP-03 contract/workspace checks and 33 C8 metric/semantic/standards/envelope checks, including parity negatives and nested-byte enforcement; both CLI gates pass. The scorer reports `synthetic_compatibility_only`, `promotion_ready=false` and `quality_thresholds_calibrated=false`. Earlier policy-2.0.0 captures remain historical. The development-only validator is reused from ignored `.codex-tmp`; it is not a plugin dependency. This compatibility evidence does not prove actual CP-09 routing or recommendation quality.
...
capacity ceilings are owner-accepted, while relevance, token, latency and usefulness remain uncalibrated. The scenario/result formats and scorer now pass
