...
`CP-03-C9-V2` has updated the active card/activity/query/result/evidence-pack/manifest/policy schemas plus linked fixtures and repeated the C8 compatibility join. CP-06 next adds `tests/test_plugin_catalog.py` and `tests/test_plugin_search_index.py`; CP-09 adds `tests/test_plugin_retrieval.py` alongside matching tests. Exact remaining future files/commands are registered in [CP-02 verification](specs/decisions/plugin-v1-verification.md) and the detailed CP tasks. CP-04 owns `evals/plugin-v1/evaluate_retrieval.py`, `tests/test_plugin_retrieval_eval.py` and `tests/fixtures/plugin_retrieval_eval.json`: current results prove offline v2 synthetic compatibility only. Actual retrieval captures, relevance judgments and calibration remain CP-04 work; this is not a runtime retrieval/model runner. Require nonzero expected test counts; unittest discovery with zero tests is not acceptance.
