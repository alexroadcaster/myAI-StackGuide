...
| P2: local useful vertical slice | CP-06-CP-11 | CP-06 build/package and bounded CP-07 intake/state/preflight are implemented. CP-08 is `completed_local_v13` with four-root mode evidence, task-specific context on a 139,618-path root and a 100,000-record fresh-process checkpoint restore. Actual read-only v2 retrieval, rendered workspace and the joined integration report remain CP-09-11. |
...
- CP-03's recorded non-C9 contracts remain accepted. Its active C9 card/activity/query/result/evidence-pack/manifest and retrieval-policy surface now has one bounded v2 migration gate; remote C4/C7 stay deferred to CP-12.
- CAT-08/09/10 are complete at their distinct evidence ceilings. CAT-08 reconciled the 2,500-identity candidate and old-card/addition field map, CAT-09 verified all identities/routes plus representative offline browser behavior, and CAT-10 froze the exact source/output pins and migration handoff. CP-06 consumed only that frozen version and produced the paired build-verified bundle.
- CP-06 consumed the exact CAT-10 snapshot after v2 contract acceptance, emitted only self-contained card-2.0.0 records, preserved numeric identity/source lineage/historical names/dual descriptions/typed assignments/unknowns and built the paired public-only policy-2.1.0/index-format-2 assets. The 2026-09-21 owner-accepted capacity amendment regenerated the policy-paired manifest/index boundary without changing the 2,500 card bytes or proving CP-09 relevance. Public card facts are not split into a metadata sidecar. This build evidence does not establish CP-09 routing or relevance.
- CP-09 must accept only paired v2 pins, keep candidate/model-input budgets fixed and report incompatibility/unavailability separately from no-match. CP-04/11/15 measure multilingual relevance, historical-alias and upstream-versus-catalog-description behavior and capacity on the actual 2,500-card snapshot, with a separately labeled 10,000-row synthetic fixture only for headroom.
...
- CP-15 reviews the local v2 product; CP-16 verifies clean install, paired card/policy/index compatibility, read-only search, state preservation and complete-package rollback. Mixed v1/v2 sets fail explicitly. Neither waits for live auth/ledger/scheduler.
