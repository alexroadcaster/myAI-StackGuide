...
Document status: the detailed eight-view desktop/laptop design and RU-EN implementation scope are `owner_accepted`. CP-01/02 are implemented documentation; CP-03 addendum and earlier CP-04 C8/scorer evidence remain contract-level history; CP-04 quality calibration remains open. CP-05 is owner-accepted at `measured_local`, CP-07 is `implemented`, CP-08 is `completed_local_v13`, and CP-09 is `implemented_verified_local` on index format 3. CP-10/11 still own presentation and the complete joined lifecycle. No relevance, human usefulness, browser, remote integration, release or publication claim follows from CP-09 completion.

## 1. Goal, Sources, And Boundaries

Owner revision: 2026-09-01, with CP-07/08 closure accepted 2026-09-21 and CP-09 local implementation verified 2026-09-22. SQLite FTS5/BM25 is the local route. The owner-approved CP-03/06 prerequisite expansion migrated the active bundle from historical index format 2 to format 3 with a source-owned taxonomy route registry. CP-09 now has current read-only retrieval/matching evidence across all routes, caps, matched fields, provenance and typed failures, plus current CP-03/C8/scorer gates through the pinned project-local development environment. Product evidence remains `partially_verified`: CP-04 relevance calibration, RU/EN, browser, CP-11, human acceptance, installation and release remain open; mixed primary-before-reference interaction is a non-blocking residual. CP-12-CP-14 remain deferred, and no remote, MCP, vector or deployment path is active.
...
CP-10-A-D are sequential checkpoints within one task, not new CP IDs. Runtime Builder owns template/dictionaries/rendering; Quality Evaluator owns `tests/test_plugin_artifact.py` and browser evidence. Any shared intake/state/skill wiring remains under CP-07 ownership with sequential handoff, not parallel overlapping edits. Existing 150-hit/12-card/160-KiB retrieval and 2 MiB state/5 MiB HTML limits remain unchanged. No mobile layout, server, browser filesystem bridge, automatic integration or new translation provider is added.
...
Schema/artifact registry: local CP-03 and bounded C8 files now pass full contract compatibility verification; full CP-04 quality calibration remains open. Deferred remote and future runtime paths are owned outputs, not claimed implementations:
...
- R15 C6 addendum / implemented, partially verified: `specs/artifact/localized-presentation.schema.json` and `specs/artifact/publication-result.schema.json`, joined to affected intake/Brief/corrections/memo/plan/state schemas. `specs/artifact/session-workspace-contract.md` maps all 49 subsections and defines version, language, source, publication and semantic checks. Local schema count is now 22; legacy fixture records remain unchanged. New writes use 1.1.0; legacy reads never silently migrate.
...
- V-RETRIEVAL / CP-09: `test_plugin_retrieval.py` and `test_plugin_matching.py` now pass the structural/runtime gate for actual FTS5, query grammar, aliases, stable ordering/constraints/caps, no-hit versus unavailable, read-only and mismatch cases. RU/EN semantic equivalence and relevance remain CP-04/10/11/15 evidence.
...
The task contracts below are amended to the 2026-09-01 local scope, the owner-accepted 2026-09-21 CP-07/08 boundaries and the 2026-09-22 CP-09 structural/runtime acceptance. CP-01/02 completion reports and earlier format-2 CP-03/C8 results remain historical evidence. CP-05 through CP-09 are implemented or accepted at their stated ceilings; CP-04 quality calibration and CP-10 onward remain open.
...
- Plan trigger: Owner revision 2026-08-31: local SQLite FTS5, relevant context, activity-aware evidence and actionable OSS integration. This task remains within the local product path.
- Validator target: detailed task blocks
- Date and time of task implementation: 31-08-2026 13:23
...
- Session workspace / R15: Implemented 1.1.0 presentation/publication and affected source contracts, with 49 mapped subsections, stored questions/scan, source hashes/evidence/literal/coverage and distinct content/storage/presentation revisions. Legacy examples remain valid read-only inputs; full standards and C8/C9 compatibility gates now pass.
...
- Rollback: Undo only owned changes; preserve prior valid state, finalized history and compatible package/index. No automatic deletion, Git reset, permission weakening or silent retrieval fallback.
- Stop conditions: Unexpected sensitive data or side effects, ownership overlap, incompatible accepted contract, unsafe containment, failed mandatory evidence or missing required external authorization. Routine relevant reads are not failures; report useful partial results when safe.
- Next step: Complete CP-04 quality corpus/baseline/thresholds and human calibration, and CP-05 readiness; then follow the existing runtime dependency graph. CP-03 contract and bounded C8/C9 compatibility gates are closed.

- Current addendum evidence: 22 local v1 schemas; all 46 CP-03 and 27 C8 checks passed, as did both scorer CLI gates. Independent reviewer repeated both full suites without skips or residual P1/P2 in that contract slice. These results remain historical evidence after the 2026-09-01 `RepositoryCardV2` decision. The active C9 card/activity/query/result/evidence-pack/manifest and policy surface now requires the separately recorded `CP-03-C9-V2` migration before CP-06; unrelated CP-03 contracts remain accepted. The Completion report below is preserved first-pass history; the Current Addendum Evidence Report owns its dated acceptance.
...
#### Current Addendum Evidence Report

The preceding Completion report is preserved first-pass history. This report records the current presentation/localization/publication addendum with full contract acceptance; runtime and quality acceptance remain separate.

- status: implemented
- what was done: Added versioned presentation/publication contracts, stored questions/scan and sourced detail fields; mapped 49 subsections; retained all original fixtures and added linked current-version examples.
- files touched / work locations: Assigned CP-03 schemas, specs/artifact/session-workspace-contract.md, tests/test_plugin_contracts.py, tests/fixtures/plugin_contracts.json and active documentation.
- technical value delivered: Explicit storage/content/presentation revisions, source/hash/evidence/literal bindings, typed publication failures and legacy/current compatibility.
- product value delivered: Future HTML has defined data for all eight views and honest partial-language/progress states; no working UI is claimed.
- actual implementation date and time: 31-08-2026 13:23
- verification evidence: 46/46 full CP-03 and 27/27 full C8 tests pass with the authorized TEMP validator; both scorer CLI gates pass for four synthetic captures. Independent review repeated both full suites without skips. The frozen 49-subsection/164-path source map and original fixtures remain unchanged; no repair was needed after installation.
- residual risks: No writer/browser/translation/FTS5 runtime or human quality proof. Full CP-04 corpus/baseline/thresholds/calibration remains open; the TEMP validator is development-only and disposable.
- follow-up: Remaining CP-04 quality corpus/baseline/thresholds/calibration and CP-05 readiness, then CP-06/07 under their accepted prerequisites.
...
- Plan trigger: Owner revision 2026-08-31: local SQLite FTS5, relevant context, activity-aware evidence and actionable OSS integration. This task remains within the local product path.
- Validator target: detailed task blocks
- Date and time of task implementation: 2026-08-31 (bounded C8 compatibility accepted; quality calibration open)
- Depends on: CP-01
- Blocks: CP-11, CP-15
...
- Acceptance criteria: Predeclare Recall@k/nDCG@k, wrong-exclusion and hard-constraint checks, total context bytes/token measurement method, latency/memory methodology and thresholds before running. Keep the owner-accepted 150-hit/12-card/160-KiB/200-KiB-input capacity caps explicit and quality-uncalibrated. Preserve 16/20 rubric and zero critical failures; calibrate authorized reads/handoff instead of blanket coding refusal. Use the actual 2,500-card CAT-10/CP-06 snapshot for capacity and relevance, retain a separately labeled 10,000-row synthetic headroom fixture, and stratify held-out judgments across the 14 containers, thin/dense leaves, baseline/expansion cohorts, aliases and secondary-category dedupe. No vectors/model provider dependency for lexical eval.
...
#### Current Compatibility Evidence Report

The preceding Completion report is preserved planning history. This report records the bounded C8/C9 join assigned with the CP-03 addendum; it does not close all CP-04 quality work.

- status: in_progress
- what was done: Added C8 scenario/capture schemas, four synthetic known-answer captures, offline scorer, rubric/runner contract and compatibility tests.
- files touched / work locations: evals/scenario.schema.json, evals/result.schema.json, registered evals/plugin-v1 files, tests/test_plugin_retrieval_eval.py and tests/fixtures/plugin_retrieval_eval.json.
- technical value delivered: Real C9 schema references, independent relevance/constraint judgments, explicit metric denominators and error distinctions, semantic parity negatives.
- product value delivered: A reproducible bounded contract/scorer check is verified; search usefulness or time savings are not measured.
- actual implementation date and time: 2026-08-31 (Europe/Moscow; bounded compatibility accepted)
- verification evidence: 27/27 full C8 tests pass with the authorized TEMP validator; both CLI gates pass for four captures, with synthetic_compatibility_only and promotion_ready=false. Independent review repeated all 27 tests; schema/case pins remain unchanged.
- residual risks: Actual catalog judgments, lexical baseline, held-out split, thresholds and human RU/EN/usefulness calibration remain open. Synthetic compatibility does not prove actual retrieval or recommendation quality.
- follow-up: Complete the remaining CP-04 quality design/calibration before CP-11/15 product quality runs; bounded C8/C9 compatibility is complete.
...
The owner accepted `RepositoryCardV2` as the only active public-card representation after the frozen CAT-10 source exposed incompatible v1 identity/alias and dual-description semantics. The existing CP-03 46-check and CP-04 27-check/CLI results remain historical v1 evidence. Catalog Architect and Quality Evaluator have now atomically migrated the active card/activity/query/result/evidence-pack/index-manifest schemas, retrieval policy, fixtures and C8 compatibility checks. Non-C9 CP-03 contracts and the CP-01-CP-16 task graph remain unchanged.

| Required v2 section | Canonical fields and semantics |
| --- | --- |
| `identity` | `github_repository_id` is a required positive integer and the only canonical dedupe/join key across card/index/result/evidence/memo/integration. `catalog_record_id` plus `merged_catalog_record_ids` retain exact source lineage inside card identity only. `full_name_aliases` contains historical `owner/name` values only; also retain current `full_name`, `url` and `identity_status`. |
| `catalog` | Exact `accepted`, `candidate`, `reference` or `benchmark` status; status source, `baseline|cat07a_expansion` membership cohort, review status and evidence stage remain independent. |
| `descriptions` | Required nullable `upstream`, `catalog` and `catalog_origin`; neither description silently replaces the other. |
| `classifications[]` | The only category-assignment representation. Every direct assignment records category ID/title/kind/parent, `primary|secondary` role and placement source. Exactly one primary assignment; containers are taxonomy nodes, not direct assignments. |
| `repository` | One normalized shape for languages, structured license, topics, structured Stack, homepage/default branch, availability/visibility, archived/fork/disabled flags and popularity signals. Popularity is triage metadata, not fit. |
| `delivery` | One normalized form/deployment/hosting representation retaining reviewed value/rationale, plus nullable `requires_server`, difficulty, lifecycle, maturity and persona. Unknown source values never become invented deployment enums. |
| `activity` | Creation, repository update, push, verified commit, verified release, sync and observation times remain distinct. Legacy array and CAT-07A object observations normalize to one typed bounded array with evidence refs; unknown is null, never copied from another date. |
| `advisory` | Recommendation reason, best-for, trade-offs, use cases, avoid-if, adoption status/mode/stages, complexity, integration surface, compatibility, eligibility, recommendation status and gaps. Advisory completeness never changes catalog membership/status automatically. |
| `provenance`, `evidence`, `corpus_kind` | Bounded public source lineage, frozen pins and field-addressable evidence live in the card itself. No public card fact is stored only in a metadata sidecar; no user/project context, browser observations or raw local audit path enters cards/index. |

FTS v2 is a reproducible projection, not an alternative card schema. It uses explicit columns and accepted initial uncalibrated weights. At the 2026-09-01 contract gate, card/activity `2.0.0`, policy `2.0.0` and index format `2` were the accepted historical tuple. The current active tuple is query/result/evidence/manifest/memo `2.1.0`, request `1.1.0`, card/activity `2.0.0`, policy `2.1.0`, index format `3` and route registry `1.0.0`. No active v1/format-2 fallback, mixed-pin coercion, automatic rebuild or whole-catalog prompt fallback is permitted.

Completion status: `implemented_verified` at the synthetic contract-compatibility ceiling. Current evidence is 46/46 CP-03 checks, 28/28 C8 checks and both v2 scorer CLI gates. The scorer reports `retrieval_score_v2`, `passed=true`, `promotion_ready=false`, `quality_thresholds_calibrated=false` and `verdict=synthetic_compatibility_only`. CP-06 is unblocked, but no 2,500-card output, SQLite index, actual retrieval, relevance quality, runtime, release or publication is claimed by this gate.
...
- Output artifacts: Pipeline Builder owns scripts/build_plugin_catalog.py, scripts/build_plugin_search_index.py, data/plugin_advisory_seed.json, plugins/myai-stackguide/assets/catalog.snapshot.json, catalog.search.sqlite, catalog.search-manifest.json and retrieval-policy.json under that assets directory. Curator owns research/plugin-v1-advisory-evidence.json. Quality Evaluator owns tests/test_plugin_catalog.py and tests/test_plugin_search_index.py; sequential handoffs for shared sources/assets. No public-fact metadata sidecar is produced.
...
- Plan trigger: Owner revision 2026-08-31: local SQLite FTS5, relevant context, activity-aware evidence and actionable OSS integration. This task remains within the local product path.
- Validator target: detailed task blocks
- Date and time of task implementation: 2026-09-21T15:11:26+03:00
...
- Short description: Collect a safe bounded overview, derive an evidence-backed project topology and select the smallest goal-relevant source context needed for a correctable Brief.
- Technical value: Explicit coverage, contained reads, deterministic service/dependency topology and explainable source selection without scanning or indexing the entire project.
- Product value: Understand real integration points and connected components of an ordinary project or monorepo while showing what remains unknown.
- Scope: Plugin Runtime Builder owns plugins/myai-stackguide/scripts/scanner.py and context.py; Quality Evaluator owns tests/test_plugin_scanner.py and synthetic tests/fixtures/plugin_scanner/. Consume C2/C3 and CP-07 state helpers; do not rewrite their contracts during implementation. Implement a transient standard-library typed topology after safe parsing, with evidence-backed project/service/manifest/package/API/storage/deployment/test nodes and relations. Required bounded algorithms are weak/strong components, cycle condensation, topological generations and goal-seeded ancestor/descendant traversal; persist only existing topology facts, basis-linked inferences, gaps and safe references.
- Mode contract: `standard` is the default when the user requests a scan without naming a mode; `quick` is an explicitly requested fast overview; `deep` is a bounded continuation that requires a visible material gap and `explicit_confirmation=true` for every transition from a lower mode. Direct `quick -> deep` is valid and does not require `standard`, but has the same confirmation gate. Classification never selects/escalates a mode. Continuation preserves the same run/root/evidence and cumulative file/byte/active-time counters; user/Codex waiting time is excluded. Owner-accepted policy v1.3 totals are quick 2,000 files/256 MiB/120 s, standard 30,000/2 GiB/1,200 s and deep 100,000/8 GiB/4,000 s. V1.3 retains the v1.2 aggregate limits and adds generated/runtime/cache/worktree exclusions observed on the large calibration root. Mode-specific topology ceilings remain 50,000/depth 20/20 s, 250,000/depth 40/90 s and 1,000,000/depth 50/300 s; per-file ceilings remain 1/2/4 MiB. Targeted context remains quick 16 paths/1 MiB/24 KiB model-facing, standard 64/8 MiB/48 KiB and deep 256/64 MiB/64 KiB, additionally bounded by the selected mode's remaining cumulative budget. Automatic whole-file selection keeps a 4 KiB minimum model share per source; explicit bounded line ranges may use the larger path ceilings. Warn at 80%; checkpoint `deep` after at most each 2,000 files or 256 MiB. Exhaustion produces a gap, not an automatic retry/reset.
- Non-goals: Unassigned files, model/permission changes, Git history, automatic installs, private-data disclosure or external activation. No remote architecture/runtime, vector/embedding dependency, GraphRAG, graph database, persisted raw project graph, automatic source indexing or shipped NetworkX dependency.
- Expected result: Empty/compact/standard/large/monorepo scans, explicit mode/counters/completion reasons, goal-relevant graph-guided selection requests and corrected versioned Brief. Structured scanner output and transient relevant excerpts are distinct paths; persisted artifacts contain minimized findings/references. `deep` remains bounded by its mode-specific topology ceiling and the common sensitive-source policy and is never described as exhaustive.
- Acceptance criteria: Test the default/explicit mode rules, cumulative quick-to-standard-to-deep escalation, no automatic escalation/reset, exact caps and one-over, targeted-context remaining-budget calculation, monorepo precedence/incomplete traversal, deterministic graph results and stable path tie-breaks, connected/disconnected/cyclic service graphs, condensation before topological generations, goal-seeded selection, partial-graph unknowns, cancellation/encoding/changed files, sensitive exclusions, Windows links/junctions/ADS/hardlinks/path swaps and output redaction. Relevant authorized code reads succeed; disallowed secrets/scope expansions fail. No project import/script/build/install/network. A failed scanner cannot bypass the same containment/exclusion rules via a different tool. Graph-derived architecture claims remain basis-linked inferences unless directly observed.
- Verification gates: Targeted test_plugin_scanner.py plus named quick/standard/deep measurements on representative idea/compact/ordinary/large-or-monorepo fixtures, observed useful authorized context and adversarial no-execution/leakage cases. Record entries/files/bytes/active time/coverage and targeted-source usefulness; compare against thresholds only after a measurement method is declared. No host-wide isolation, performance SLA, calibrated-limit or universal-project-coverage claim.
- Risks / approval gates: Preserve pre-existing dirty work and source-owned boundaries. Public research is read-only; sensitive scope, credentials, material cost, external writes, publication and destructive operations follow actual authorization. Policy-v1.3 mode evidence covers four real roots, including a 139,618-path workspace reduced to 1,637-1,641 relevant attempts after exclusions. A synthetic 100,000-record checkpoint fits the 64 MiB private slot and restores in a fresh process. This is local scale/recovery evidence, not a performance SLA, exhaustive arbitrary-project scan or human product acceptance. The 1,000,000-entry deep topology cap can still leave a very large repository partial; absent edges and unvisited areas remain explicit unknowns.
- Complexity: L
- Estimated execution time: Re-estimate at dispatch from accepted inputs and the first bounded slice; earlier ranges are superseded by the changed FTS5/context/integration scope. No delivery commitment or provider-cost estimate.
- Agents: Owner proposed plugin_runtime_builder; test owner quality_evaluator; reviewer evidence_reviewer.
- Skills: build-stackguide-plugin after CP-05; design-context-contracts; audit-readonly-boundaries.
- Output artifacts: Plugin Runtime Builder owns `plugins/myai-stackguide/scripts/scanner.py`, `context.py` and the scan/context adapter in `intake.py`; Catalog Architect owns the checkpoint and commit-request schemas plus aligned artifact/team contracts; Quality Evaluator owns `tests/test_plugin_scanner.py` and `tests/test_plugin_intake.py`. The minimized calibration artifact is `docs/reports/cp08-real-project-calibration-2026-09-21.json`.
- Evidence owner: quality_evaluator; traces contain counts/reason codes, not source text.
- Docs update path: Product Planner updates PLAN.md/task status; Quality Evaluator records TEST.md/EVALS.md evidence; Docs Maintainer appends RUNLOG.md after handoff. No unsupported completion claim.
- Rollback: Undo only owned changes; preserve prior valid state, finalized history and compatible package/index. No automatic deletion, Git reset, permission weakening or silent retrieval fallback.
- Stop conditions: Unexpected sensitive data or side effects, ownership overlap, incompatible accepted contract, unsafe containment, failed mandatory evidence or missing required external authorization. Routine relevant reads are not failures; report useful partial results when safe.
- Next step: CP-09 retrieval and CP-10 reporting may consume the closed scanner/Brief seam; CP-11 owns the joined lifecycle, CP-15 human/privacy/product acceptance and CP-16 package/release evidence.

#### Completion report

- status: completed_local_v13
- what was done: Implemented standard-library bounded scanning, cumulative mode continuation, minimized reports, transient typed topology, goal-seeded references-only selection, versioned Brief composition, canonical correction/invalidation, CP-07 `commit-scan`/`commit-context` adapters and private restart checkpoints. No NetworkX, vector RAG, graph database, remote service or second state writer was added.
- files touched / work locations: `plugins/myai-stackguide/scripts/scanner.py`, `context.py`, `intake.py`; `tests/test_plugin_scanner.py`, `tests/test_plugin_intake.py`; checkpoint/commit request schemas, artifact/team contracts and the minimized real-project calibration report.
- technical value delivered: The local plugin now has a bounded producer-to-writer seam with cumulative counters, safe exclusions, deterministic graph algorithms, minimized evidence, restart/replay validation, mode-nested context budgets and dependency invalidation after corrected Brief changes.
- product value delivered: A selected local project can be summarized and committed into a correctable, versioned Brief without executing project code or persisting excerpts. Four real roots demonstrate bounded mode behavior; the large-root task-specific run selected direct configuration, implementation and migration evidence across contact graph, local RAG, messaging, search and integration concerns.
- actual implementation date and time: 2026-09-21T15:11:26+03:00
- verification evidence: Historical bounded producer/writer and three-root evidence remains recorded. Closeout added policy-v1.3 generated/runtime exclusions, checkpoint/storage schema 1.1 with a 64 MiB private slot, one 100,000-record fresh-process restore, all-mode observations on the 139,618-path root and a task-specific relevance repair. Final current-run evidence passed scanner/intake 18/18, all Draft 2020-12 plus semantic/workspace contracts 48/48 and Codex control-plane contracts 13/13. The task-specific rerun selected 12/12 direct goal matches and fairly delivered 42,882 model-facing bytes across configuration, README, implementation and migrations.
- residual risks: Persisted crash injection, host-wide isolation, universal secret detection, universal filesystem-race resistance, arbitrary 100,000-source-file performance and human product acceptance are not claimed. Partial non-budget gaps remain explicit. `policy_limits_calibrated=false` denotes the absence of a universal SLA, not unfinished CP-08 implementation. Remote extension and vectors are not prerequisites.
- follow-up: CP-09/10 may consume the current scanner/Brief seam. CP-11 proves the joined lifecycle; CP-15 owns human/privacy/usefulness acceptance; CP-16 owns installation, rollback and release evidence.
...
- Plan trigger: Owner revision 2026-08-31: local SQLite FTS5, relevant context, activity-aware evidence and actionable OSS integration. This task remains within the local product path.
- Validator target: detailed task blocks
- Date and time of task implementation: 2026-09-22; exact time not recorded
...
- Scope: Plugin Runtime Builder implemented `retrieval.py`, `context_pack.py` and `matcher.py`; Quality Evaluator owns the two focused test modules. The owner-approved prerequisite expansion separately allowed Catalog Architect/Pipeline Builder/Quality Evaluator to migrate the required C9 contracts, index builder, generated bundle, intake trust pins and fixtures to format 3. That expansion is closed and does not transfer broad builder or CP-07 state ownership to CP-09.
- Non-goals: Unassigned files, model/permission changes, Git history, automatic installs, private-data disclosure or external activation. No remote architecture/runtime or vector/embedding dependency.
- Expected result: Delivered at the structural/runtime ceiling: read-only `sqlite_fts5` route with full tuple validation, bounded compiled queries, RRF over weighted BM25 variants, numeric-ID dedupe, exact matched fields, eligibility checks and bounded evidence packs.
- Acceptance criteria: Accepted for the active format-3 tuple across all 111 leaf, 14 container and one review route, 162 memberships, assignment dedupe, request-selected caps, exact matched fields, snapshot/full-manifest provenance and explicit unavailable/corrupt/incompatible failure states. No index write, rebuild, full-catalog fallback, embedding, remote call or hidden query logging is accepted.
- Session workspace / R15 (open downstream): CP-10/11 must prove that display-language switching preserves the canonical query, pack, candidate order and pins without another retrieval call.
- Verification gates: Current project-local evidence is 23/23 retrieval/matching, 18/18 index/intake, 48/48 CP-03 contracts, 33/33 C8 checks, two passing scorer CLI gates and a passing builder `--check`; independent review reports no remaining S1 runtime issue. The scorer still declares `synthetic_compatibility_only`, `promotion_ready=false` and `quality_thresholds_calibrated=false`, so semantic relevance gains remain CP-04/15 gates rather than unfinished CP-09 implementation.
- Risks / approval gates: CP-09 is `implemented_verified_local`. Mixed primary-before-reference interaction has not been observed on trusted data and is a non-blocking residual. Relevance, RU/EN, browser, CP-11, human and release evidence remain open; no remote/MCP/vector/deployment activation is authorized or implied.
- Complexity: M
- Estimated execution time: Historical estimate superseded; actual duration was not reconstructed from partial handoffs.
- Agents: Plugin Runtime Builder implemented runtime; Quality Evaluator owned tests/evidence; Catalog Architect and Pipeline Builder owned the approved prerequisite migration; Evidence Reviewer remained read-only.
- Skills: `build-stackguide-plugin`, `design-catalog-contracts` and `design-recommendation-evals` governed the implemented slice; `maintain-control-plane` governs this status reconciliation.
- Output artifacts: Runtime modules and focused tests above, plus the owner-approved sequential prerequisite migration of the active C9 schemas, index builder, package assets, intake trust pins and fixtures. Regular owners retain those shared surfaces after handoff.
- Evidence owner: quality_evaluator; product_planner for usefulness.
- Docs update path: Product Planner updates PLAN.md/task status; Quality Evaluator records TEST.md/EVALS.md evidence; Docs Maintainer appends RUNLOG.md after handoff. No unsupported completion claim.
- Rollback: Undo only owned changes; preserve prior valid state, finalized history and compatible package/index. No automatic deletion, Git reset, permission weakening or silent retrieval fallback.
- Stop conditions: Unexpected sensitive data or side effects, ownership overlap, incompatible accepted contract, unsafe containment, failed mandatory evidence or missing required external authorization. Routine relevant reads are not failures; report useful partial results when safe.
- Next step: CP-04/Quality Evaluator calibrates relevance from the now-running scorer path; CP-10 and CP-11 own RU/EN/browser presentation and the joined local lifecycle; CP-15/16 own human and release evidence. Remote merge remains optional CP-13.

#### Completion report

- status: implemented_verified_local
- what was done: Implemented the local retrieval/matching/context-pack runtime and the explicitly approved CP-03/06 prerequisite migration to the active format-3 bundle.
- files touched / work locations: Runtime modules, focused tests and the approved C9 contract/index/intake/package compatibility unit; this plan update does not alter those implementation bytes.
- technical value delivered: The immutable 2,500-card catalog now has a validated read-only FTS5 route with source-owned taxonomy routing, deterministic fusion/dedupe, bounded evidence and fail-closed tuple/provenance checks.
- product value delivered: Structural retrieval can produce a bounded, inspectable candidate/evidence set; recommendation usefulness is not yet accepted.
- actual implementation date and time: 2026-09-22; exact time not recorded
- verification evidence: 23/23 retrieval/matching, 18/18 index/intake, 48/48 CP-03 contracts, 33/33 C8 checks, both scorer CLI gates and builder `--check`; 126 routes/162 memberships, inherited generated-asset ACLs and independent no-S1 review.
- residual risks: Relevance, mixed primary-before-reference observation, RU/EN, browser, joined CP-11, human and release evidence remain open. The scorer is intentionally `synthetic_compatibility_only`; no promotion-ready, remote, MCP, vector or deployment claim.
- follow-up: CP-04 relevance calibration, then CP-10/11 joined presentation/runtime evidence and CP-15/16 acceptance/release.

### Task `CP-10`

- Task: CP-10 — render offline decisions with integration plan and coding handoff
- Status: in_progress; CP-10-A implemented, partially_verified (browser checkpoint pending)
- Schema version: task_matrix_plan_v1
- Timezone: Europe/Moscow
- Plan trigger: Owner revision 2026-08-31: local SQLite FTS5, relevant context, activity-aware evidence and actionable OSS integration. This task remains within the local product path.
- Validator target: detailed task blocks
...
- Non-goals: Unassigned files, model/permission changes, Git history, automatic installs, private-data disclosure or external activation. No remote architecture/runtime or vector/embedding dependency.
- Expected result: A named 1/1 useful synthetic project scenario with actual packaged FTS5 query and artifact; then focused edge/regression checks, actual 2,500-card catalog capacity measurements and separately labeled 10,000-row synthetic headroom.
- Acceptance criteria: Record Brief/source/card-2.0.0/index-format-3/policy-2.1.0 versions and hashes, source_mode=catalog_only, retrieval_engine=sqlite_fts5, numeric result IDs, matched v2 fields, counts/bytes, exclusions and integration handoff. Prove an alias lookup and a CAT-07A catalog-description lookup through the actual packaged route. Verify corrections/resume, no-match versus index failure, mixed-pin incompatibility, activity unknowns, sparse cards, no network/install/private-index writes, offline renderer and state recovery. Measure build/index size and cold/warm behavior on the actual 2,500-card snapshot; use 10,000 synthetic rows only for headroom. Synthetic duplicates cannot prove recall or genuine catalog growth; runtime pass cannot prove model quality.
...
- Acceptance criteria: Use CP-04 v2-compatible cases/runner/judgments and the actual CP-11 FTS5 v2 route bound to the 2,500-card snapshot. Inspect RU/EN/current-name/full-name-alias and upstream-versus-catalog-description cases, all container semantics through deterministic checks, representative thin/dense leaves, baseline/expansion cohorts, typed secondary-assignment dedupe, mature-old versus active-incompatible cases, missing facts/no-match, integration prerequisites and rollback. Verify numeric identity and in-card provenance survive the evidence-pack budget. Avoid a random corpus dominated by the largest expansion leaves. Permit useful ordinary source reads and proposed implementation steps; reject unauthorized execution/disclosure and unsupported operability claims. No CP-12/13/14 dependency. Record model/provider authority and evidence separately if a model run is needed.
...
- Acceptance criteria: Actual supported Windows/Python/FTS5 preflight, installed/cache identity, read-only v2 query, no network/download, state compatibility and previous-package/index rollback verified in an authorized synthetic workspace. Record package, 2,500-card snapshot, card-2.0.0/policy-2.1.0/index-format-3 and actual asset sizes/hashes; verify upgrade from the prior 1,800-card/126-node source package to the paired 2,500-card/126-node v2 package and rollback to one complete compatible prior package. A v1 or partially rolled-back state/card/index/policy set must fail explicitly rather than coerce or rebuild. No private artifacts or secrets in package. Owner accepts local scope/evidence; publication needs exact separate authorization and is not required to label local package-ready.
