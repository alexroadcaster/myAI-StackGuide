...
- Preserve the CAT-10 frozen snapshot and its CAT-09 verified projection: 2,500 repositories, 126 taxonomy nodes and 2,630 placements. The [final freeze report](docs/reports/catalog-final-freeze-2026-09-01.json) binds the exact source/schema/taxonomy/field-contract/output hashes and CP-06 handoff. CP-06 owns normalized metadata/card/index adapters from that exact version; browser observations are not persisted source facts. Source changes require a new version, regeneration and parity checks rather than silent drift of these pins.
