...
The owner accepted `RepositoryCardV2` as the only active public-card representation after the frozen CAT-10 source exposed incompatible v1 identity/alias and dual-description semantics. The existing CP-03 46-check and CP-04 27-check/CLI results remain historical v1 evidence; they do not validate v2. Before CP-06 writes generated assets, Catalog Architect and Quality Evaluator must atomically migrate the active card/activity/query/result/evidence-pack/index-manifest schemas, retrieval policy, fixtures and C8 compatibility checks. Non-C9 CP-03 contracts and the CP-01-CP-16 task graph remain unchanged.

| Required v2 section | Canonical fields and semantics |
| --- | --- |
| `identity` | `github_repository_id` is a required positive integer and the only canonical dedupe key across card/index/result/evidence. `catalog_record_id` plus `merged_catalog_record_ids` retain exact source lineage. `full_name_aliases` contains historical `owner/name` values only; also retain current `full_name`, `url` and `identity_status`. |
...
FTS v2 is a reproducible projection, not an alternative card schema. It uses explicit columns and accepted initial uncalibrated weights: `full_name=5`, `full_name_aliases=5`, `upstream_description=3`, `catalog_description=3`, `topics=3`, `category_labels=1`, `use_cases=3`, `integration_surface=3`, `best_for=2`. After the migration gate passes, the required target paired set is card `2.0.0`, retrieval policy `2.0.0` and index format `2`; query/result/evidence-pack/manifest fixtures must carry compatible pins. The checked-in schemas/policy remain v1 until this work is implemented and verified. No active v1 fallback after cutover, mixed-version coercion, automatic rebuild or whole-catalog prompt fallback is permitted.
...
- Rollback: Undo only owned changes; preserve prior valid state, finalized history and compatible package/index. No automatic deletion, Git reset, permission weakening or silent retrieval fallback.
- Stop conditions: Unexpected sensitive data or side effects, ownership overlap, incompatible accepted contract, unsafe containment, failed mandatory evidence or missing required external authorization. Routine relevant reads are not failures; report useful partial results when safe.
- Next step: Complete `CP-03-C9-V2` schema/policy/fixture migration and v2 C8 compatibility gate, then CP-06 builds cards/index. CP-09 consumes only the paired v2 cards/index/policy after CP-07. Public evidence collection must stay within its assigned read-only scope; incomplete coverage is reported, not fabricated.
