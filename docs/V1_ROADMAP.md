...
- CP-06 consumed the exact CAT-10 snapshot after v2 contract acceptance, emitted only self-contained card-2.0.0 records, preserved numeric identity/source lineage/historical names/dual descriptions/typed assignments/unknowns and built the paired public-only policy-2.0.0/index-format-2 assets. Public card facts are not split into a metadata sidecar. This build evidence does not establish CP-09 routing or relevance.
