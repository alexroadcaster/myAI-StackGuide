...
After the atomic `CP-03-C9-V2` contract gate, CP-06 consumes only the exact CAT-10 2,500-record snapshot and emits one self-contained `RepositoryCardV2` per positive numeric `github_repository_id`; no public card fact lives only in a metadata sidecar. Exact catalog IDs are lineage, historical `owner/name` aliases are separate searchable names, upstream and catalog descriptions remain distinct, and one typed classification-assignment list preserves role/kind/parent/source. Normalized repository, delivery, activity, advisory, provenance and evidence sections preserve unknowns plus baseline/expansion lineage without browser enrichment.
