...
The owner accepted CP-08 scan-policy v1.1 ceilings on 2026-09-21. With the existing development-only validator path, `python -B -m unittest tests.test_plugin_contracts -v` passed **48/48**, including the new exact mode-specific policy assertion and all linked schema/fixture checks. `python -B -m unittest tests.test_codex_contracts -v` passed **13/13**, and six changed JSON-compatible policy/schema/fixture files parsed successfully. This is contract/static evidence only: scanner runtime, containment, representative real-project measurements, limit calibration and graph-guided usefulness remain unverified; `policy_limits_calibrated=false`.
...
| `V1-SCAN-001` | Allowed docs/config sources are included and reported under exact CP-08 v1.1 quick 250 files/32 MiB/60 s, standard 2,000/256 MiB/480 s and deep 10,000/2 GiB/1,800 s cumulative ceilings; topology, per-file and targeted-context caps scale by mode. | One-over boundaries, 80% warning, deep checkpoint, secrets, keys, dumps, logs, generated dependencies, automatic escalation/reset, and executable behavior are rejected or reported partial as specified. | Quality Evaluator |
...
| CP-07/08 / R02-R05, R10 | Saved/resumed/corrected Brief; `standard` default, explicit `quick` and gap-backed explicit `deep`; exact v1.1 read/topology/per-file/targeted-context ceilings; cumulative counters, 80% warnings, deep checkpoints; bounded transient topology and useful goal-guided ordinary source context | Secret exclusions, path escape/races, hostile instructions, automatic mode escalation/reset, checkpoint budget reset, graph inference promoted to fact, partial graph treated as independence, cancellation/one-over caps, stale revisions and interrupted writes; no target-project execution |
