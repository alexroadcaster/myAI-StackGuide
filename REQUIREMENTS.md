...
Owner revision: 2026-09-01, with CP-07 closure and CP-08 local closure accepted on 2026-09-21. SQLite FTS5/BM25 is selected for local catalog retrieval. The product optimizes time to a useful open-source integration or modernization plan. Relevant project reads are allowed within existing permissions; the former host-wide source-isolation requirement is superseded, not technically proven. Repository activity and evidence observation are separate; the former 30-day snapshot rejection is withdrawn. CP-01/02 remain completed documentation records. CP-03 v1 contract verification and the bounded CP-04 C8/C9 v1 join remain historical evidence. The owner accepted `RepositoryCardV2` as the only active public-card representation; `CP-03-C9-V2` is implemented and contract-verified. CP-06 generated and build-verified exactly 2,500 v2 cards and the paired SQLite FTS5 v2 bundle. CP-05 is complete at `measured_local`. CP-07 is implemented at its bounded local writer/intake/preflight ceiling. CP-08 is `completed_local_v13`: policy v1.3 enforces the accepted 2,000/30,000/100,000-file ceilings and generated/runtime exclusions; local evidence covers four real roots including one 139,618-path workspace, task-specific context selection, a 100,000-record checkpoint restored in a fresh process, Draft 2020-12 contracts and the CP-07 join. `policy_limits_calibrated=false` remains because these ceilings are not a universal SLA. Persisted crash injection, joined recommendation behavior, human acceptance, install and release are owned by CP-11/15/16. CP-04 quality calibration continues separately. CP-12-CP-14 remain deferred. Actual CP-09 retrieval routing/relevance, CP-10 UI, joined CP-11 behavior, install, release and publication remain unverified. Permissions are unchanged.

Product state is `partially_verified`: CP-06 build/package evidence and completed-local CP-07/08 scanner/state/Brief evidence are not retrieval quality, rendered UI, complete joined lifecycle or release evidence. CP-03 supplies local schemas/policies/examples and the presentation/publication contract; CP-06 supplies the deterministic public card/index bundle; CP-07 supplies the stable local state interface; CP-08 commits minimized scan/context and corrected Brief state through that sole writer. CP-04 quality calibration and CP-09 onward product/runtime work remain open; human acceptance and packaging remain CP-15/16.
...
| R06 | Local lexical RAG: one self-contained `RepositoryCardV2` -> explicit SQLite FTS5 v2 projection -> bounded evidence pack -> Codex comparison. Never load the entire catalog into model context; remote discovery is deferred. | CP-03, CP-04, CP-06, CP-09, CP-11 |
| R07 | Positive numeric `github_repository_id` is canonical across card/index/result/evidence; catalog IDs are lineage and historical `owner/name` aliases are separate. Keep upstream/catalog descriptions and creation/update/push/verified commit/release/sync/observation dates distinct. | CP-03, CP-06, CP-09, CP-10 |
| R08 | One public card separates exact catalog status, evidence stage, public provenance, advisory eligibility/completeness and request-specific role; `reference` and `benchmark` remain distinct and curator-only acceptance is unchanged. Automatic public candidate overlay is deferred. | CP-03, CP-06, CP-09, CP-12, CP-13 |
...
| R11 | Pin the paired card `2.0.0`, retrieval-policy `2.1.0`, index-format `3`, catalog and artifact hashes; reproduce candidate selection and fail closed on any mixed or stale active pin set. Remote overlay/compaction remains deferred. | CP-03, CP-06, CP-09, CP-11, CP-13, CP-16 |
...
| `V1-CAT-001` | Define one self-contained public `RepositoryCardV2` with canonical numeric identity, source lineage, dual descriptions, typed classifications, normalized repository/delivery/activity/advisory data and in-card public provenance/evidence. | `docs/PRODUCT_REQUIREMENTS.md` FR7/FR13 and Data Requirements | All 2,500 CAT-10 records map losslessly to one required v2 shape; memo/integration joins use the same numeric identity while catalog IDs remain in-card lineage; no public-fact sidecar or active v1 fallback; mixed card/policy/index versions fail closed. | Catalog Architect | `implemented_verified_cp06_build_package` |
...
Current owner-authorized sequence: implement CP-09 retrieval against the accepted CP-06 bundle and CP-10 presentation against accepted fixtures, while CP-04 quality calibration continues as a disjoint lane before the CP-11 join. PLAN.md owns exact outputs and gates. Preserve R01-R15, historical evidence and unrelated work.
...
- Preserve the CAT-10 frozen snapshot and its CAT-09 verified projection: 2,500 repositories, 126 taxonomy nodes and 2,630 placements. The [final freeze report](docs/reports/catalog-final-freeze-2026-09-01.json) binds the exact source/schema/taxonomy/field-contract/output hashes and CP-06 handoff. CP-03-C9-V2 first owns the active card/activity/retrieval schema and fixture migration; CP-06 then generates only self-contained v2 cards and a v2-only index from that exact source. Browser observations are not persisted source facts. Source changes require a new version, regeneration and parity checks rather than silent drift of these pins.
...
- CP-03 / Catalog Architect: non-C9 v1 contracts remain accepted; the active C9 card/activity/query/result/evidence-pack/manifest set migrates atomically to v2. The accepted FTS v2 field registry and initial weights remain uncalibrated engineering defaults, not quality claims.
- CP-04 / Quality Evaluator + Product Planner: first revalidate the captured-result scorer and fixtures against one consistent v2 card/policy/index pin set, then complete held-out relevance judgments, lexical baseline, exact command contract, calibrated thresholds and human rubric. Bind actual captures to the CAT-10/CP-06 2,500-card snapshot and stratify thin/dense leaves, containers, baseline/expansion cohorts, aliases and secondary-category dedupe. A 16/20 score alone is insufficient.
...
- CP-06 / Pipeline Builder + Curator: after the v2 contract gate, consume the exact CAT-10 2,500-card snapshot, emit one lossless public `RepositoryCardV2` per numeric identity, build the explicit FTS v2 projection and prove card/index/policy parity; exhaustive recommendation narratives are not a prerequisite for a useful slice.
