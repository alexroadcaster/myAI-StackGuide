...
CP-04 first revalidates its captured-result protocol against one paired `RepositoryCardV2`/policy-2.0.0/index-format-2 fixture set, then freezes a simple lexical/filter baseline over those same cards, held-out relevance judgments and query set before comparing the selected FTS5/BM25 implementation. No embedding, vector store or model/provider call is necessary to measure retrieval. A later semantic extension must demonstrate gain over this baseline and account for installation, latency, memory and maintenance costs; it is not a current gate.
...
The [C8 runner contract](evals/plugin-v1/runner-contract.md), scenario/result schemas and scorer are present. Four independent synthetic v1 captures covered allowed, no-match, denied and missing-index outcomes. All 27 metric/semantic/standards/envelope checks passed, including CP-03/C8 parity negatives and nested-byte enforcement. Both CLI gates passed; the scorer reported synthetic_compatibility_only and promotion_ready=false. Independent review repeated that suite. This evidence remains historical until the same outcome set carries compatible card-2.0.0/policy-2.0.0/index-format-2 pins and passes again. The development validator was authorized and installed only in TEMP; see TEST.md.
