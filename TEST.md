...
The mandatory `CP-03-C9-V2` gate passed on 2026-09-01: **46/46 CP-03 tests and 28/28 C8 tests**, without skips, plus both v2 scorer CLI gates. The active fixtures now use `RepositoryCardV2`, ActivityV2, policy `2.0.0` and index format `2`; recommendation memo and integration plan also use numeric GitHub IDs, leaving catalog IDs as in-card lineage only. Tests reject mixed pins, string/zero/generic repository identities in retrieval and downstream memo/plan consumers, and unresolved or non-local schema references. The scorer output is `retrieval_score_v2` with `synthetic_compatibility_only`; no actual SQLite retrieval, 2,500-card generation, relevance calibration or runtime activation is implied.
...
| CP-03-C9-V2 / R06-R08, R11 | Atomic active-contract migration to one self-contained `RepositoryCardV2`, ActivityV2, retrieval policy 2.0.0 and index format 2; all linked query/result/evidence-pack/manifest fixtures use compatible pins | Existing 46/27 checks remain v1 history; mixed v1/v2 refs, generic `repo_id`, collapsed descriptions/statuses, split public-fact sidecar and active v1 fallback rejected |
...
| CP-11 / R12 | Named useful 1/1 path with actual source_mode=catalog_only, retrieval_engine=sqlite_fts5 and paired card-2.0.0/policy-2.0.0/index-format-2 pins; prove one historical-alias and one catalog-description route; actual 2,500-card capacity plus separately labeled 10,000-row synthetic headroom | No mock/alternate/v1 route masks acceptance; synthetic headroom cannot prove search relevance |
