...
Document status: the detailed eight-view desktop/laptop design and RU-EN implementation scope are `owner_accepted`. CP-01/02 are implemented documentation; CP-03 addendum and bounded CP-04 C8/scorer compatibility are contract-verified; CP-04 quality calibration remains open. CP-05 source alignment and its 19-case fresh-session evidence are owner-accepted at `measured_local`. CP-07 is owner-accepted as `implemented` at its bounded writer/intake/preflight ceiling. CP-08 is `completed_local_v13` with bounded producer/Brief/correction/checkpoint/sole-writer behavior, four-root measurements, task-specific context evidence and a 100,000-record fresh-process checkpoint restore. CP-09 and CP-10-A may proceed against the stable interfaces; CP-11 still owns the complete joined lifecycle. No product retrieval, browser, remote integration, release or publication claim follows from these bounded slices.

## 1. Goal, Sources, And Boundaries

Owner revision: 2026-09-01, with CP-07 and CP-08 local closure accepted 2026-09-21. SQLite FTS5/BM25 is selected for local catalog retrieval. The product optimizes time to a useful open-source integration or modernization plan. Relevant project reads are allowed within existing permissions; the former host-wide source-isolation requirement is superseded, not technically proven. Repository activity and evidence observation are separate; the former 30-day snapshot rejection is withdrawn. CP-01/02 remain completed documentation records. The recorded CP-03/C8 results remain historical evidence. The owner accepted `RepositoryCardV2` as the sole active public-card representation; CP-06 generated and build-verified the exact 2,500-card snapshot plus paired FTS v2 index/policy/manifest. CP-05-C is owner-accepted at `measured_local`. CP-07 is implemented at its bounded local writer/intake/preflight ceiling. CP-08 is `completed_local_v13`: full local regressions cover the producer/writer/Brief/checkpoint join, four real roots include a 139,618-path workspace, task-specific context ranks direct implementation evidence, and a 100,000-record checkpoint restores in a fresh process. `policy_limits_calibrated=false` remains a non-SLA flag. Persisted crash injection, joined behavior, human acceptance and release remain CP-11/15/16. CP-04 quality calibration continues. CP-12-CP-14 remain deferred. Actual CP-09 retrieval routing/relevance, CP-10 UI, joined CP-11 behavior, install, release and publication remain unverified. Permissions are unchanged.
...
The task contracts below are amended to the 2026-09-01 local scope and the owner-accepted 2026-09-21 CP-07/08 boundaries. CP-01/02 completion reports are preserved historical evidence for their original documentation runs; the amendment does not retroactively claim new tests or implementation. CP-03/04 current fields record the addendum and open acceptance gates. Their completion reports below preserve earlier first-pass/planning evidence; CP-05 source alignment, CP-06 bundle build, CP-07 local writer/intake/preflight and CP-08 local scanner/context are implemented at their stated ceilings, while CP-09 onward remain future work.
...
FTS v2 is a reproducible projection, not an alternative card schema. It uses explicit columns and accepted initial uncalibrated weights: `full_name=5`, `full_name_aliases=5`, `upstream_description=3`, `catalog_description=3`, `topics=3`, `category_labels=1`, `use_cases=3`, `integration_surface=3`, `best_for=2`. The active paired set is card `2.0.0`, activity `2.0.0`, retrieval policy `2.0.0` and index format `2`; query/result/evidence-pack/manifest fixtures carry compatible pins. Historical v1 schemas remain only for non-C9 compatibility evidence. No active v1 fallback after cutover, mixed-version coercion, automatic rebuild or whole-catalog prompt fallback is permitted.
...
- Scope: Plugin Runtime Builder owns plugins/myai-stackguide/scripts/retrieval.py, context_pack.py and matcher.py. Quality Evaluator owns tests/test_plugin_retrieval.py and tests/test_plugin_matching.py. Consume only the paired CP-06 `RepositoryCardV2`/FTS v2/policy v2 assets and compatible CP-03 contracts; do not edit catalog/evidence/index builders or duplicate CP-07 state code.
- Non-goals: Unassigned files, model/permission changes, Git history, automatic installs, private-data disclosure or external activation. No remote architecture/runtime or vector/embedding dependency.
- Expected result: Read-only sqlite_fts5 v2 route with paired version/hash/integrity validation, bounded compiled queries, weighted BM25 over explicit v2 columns, RU/EN/technology aliases, numeric-ID dedupe and task-specific roles/evidence pack.
- Acceptance criteria: Parameterize SQL and escape FTS grammar; handle malformed/empty/no-hit queries; stable ordering and a request-selected total ceiling of 150 fetched hits, 12 detailed cards, 160 KiB evidence and 200 KiB controlled input across query variants. Return numeric `github_repository_id` and field matches that distinguish current name, historical full-name alias, upstream description and catalog description. Verify all 111 leaf filters, all 14 container descendant unions, the review queue and secondary-assignment dedupe against the exact 2,500-card v2 pins. Reject v1/mixed card/policy/index/pack versions with explicit incompatible status; never coerce, rebuild or use full-catalog fallback. Constraint unknowns produce caveats/next verification, no unconditional fit; snapshot age alone never rejects. No index writes, embeddings, remote calls or hidden query logging.
...
- Acceptance criteria: Record Brief/source/card-2.0.0/index-format-2/policy-2.1.0 versions and hashes, source_mode=catalog_only, retrieval_engine=sqlite_fts5, numeric result IDs, matched v2 fields, counts/bytes, exclusions and integration handoff. Prove an alias lookup and a CAT-07A catalog-description lookup through the actual packaged route. Verify corrections/resume, no-match versus index failure, mixed-v1/v2 incompatibility, activity unknowns, sparse cards, no network/install/private-index writes, offline renderer and state recovery. Measure build/index size and cold/warm behavior on the actual 2,500-card snapshot; use 10,000 synthetic rows only for headroom. Synthetic duplicates cannot prove recall or genuine catalog growth; runtime pass cannot prove model quality.
