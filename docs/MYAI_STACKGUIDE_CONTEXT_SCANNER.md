...
Active owner revision: 2026-09-21. Read [PRD](PRODUCT_REQUIREMENTS.md#active-plugin-v1-requirements), [architecture](../specs/decisions/plugin-v1-architecture.md#scan-budgets-and-classification) and [permissions](../specs/decisions/plugin-v1-permissions.md). CP-03 provides [scan policy](../specs/scanner/scan-policy.yaml), [exclusion examples](../specs/scanner/exclusion-cases.json) and typed context contracts. Policy v1.3 retains the owner-accepted v1.2 aggregate ceilings and adds generated/runtime/cache/worktree exclusions observed on a 139,618-path project. CP-08 is `completed_local_v13`: the scanner/context producer, CP-07 commit adapters, Brief composition/corrections, task-aware context selection and 64 MiB private restart checkpoint pass current local regressions. The [large-project closeout](reports/cp08-large-project-closeout-2026-09-21.md) records four-root mode evidence, task-specific selection and a 100,000-record fresh-process checkpoint restore. Human product acceptance and release evidence remain CP-15/16.
...
Runtime status: `plugins/myai-stackguide/scripts/scanner.py` and `context.py` produce minimized scan structures, references-only selections and versioned Briefs; raw excerpts remain transient. `intake.py` validates `commit-scan` and `commit-context` requests and writes canonical state only through the CP-07 sole-writer boundary. Corrections mutate the canonical Brief domain and invalidate `selection`, `request`, `retrieval_result`, `evidence_pack` and `recommendation_memo`. A private `.scan-checkpoint.json` sidecar supports bounded restart/replay without persisting a raw topology; restoration therefore marks topology incomplete until rebuilt. HTML publication remains owned by CP-07/10/11.
...
| --- | --- | --- | --- |
| `quick` | Explicit user request for a fast overview | Early orientation from manifests and highest-signal files; incomplete coverage is expected | 2,000 file attempts, 256 MiB consumed source bytes, 120 seconds active scanner time |
| `standard` | Default when the user requests a project scan and does not name a mode | Normal decision-support scan for a compact, ordinary or moderately large project | 30,000 file attempts, 2 GiB, 1,200 seconds active scanner time |
| `deep` | Explicit user selection after Codex shows the remaining gap and explains why more local reading may help | Bounded expansion for a complex/monorepo case; never an exhaustive audit | 100,000 file attempts, 8 GiB, 4,000 seconds active scanner time |

Codex may recommend `quick` or `deep`, but cannot change the limits or silently escalate to `deep`. A user request that already names the mode is the selection; otherwise `standard` is used after the normal root/purpose/budget/exclusion disclosure. Every transition from a lower mode to `deep` requires a visible material gap and `explicit_confirmation=true`. A direct `quick -> deep` transition is valid without an intermediate `standard` pass, but has the same confirmation gate. Continuation reuses the same run, root, exclusions, visited evidence and cumulative counters. The higher profile is a cumulative total, not an additional allowance: for example, 200 attempts in `quick` leave at most 29,800 attempts after escalation to `standard`, then at most 70,000 more after a fully consumed `standard` result escalates to `deep`. Active scanner elapsed time is accumulated across scan/read phases; time spent waiting for the user or Codex is not scanner execution time. Hitting a limit returns partial coverage and a visible gap; it never triggers an automatic retry or escalation.

Topology and per-file ceilings scale with the selected mode: `quick` permits 50,000 visited entries/depth 20/20 seconds and 1 MiB per file; `standard` permits 250,000/depth 40/90 seconds and 2 MiB per file; `deep` permits 1,000,000/depth 50/300 seconds and 4 MiB per file. Topology time is charged to the selected mode total. The structured scanner response remains capped at 256 KiB. At 80% of any mode ceiling runtime emits a budget warning. A `deep` scan records a continuation checkpoint after at most each 2,000 additional file attempts or 256 MiB consumed. Policy v1.3 keeps the accepted aggregate profiles and excludes observed generated/runtime/cache/worktree trees before allowlist evaluation. Earlier three-root v1.2 runs reached 2,740-5,911 relevant attempts under standard; the fourth 139,618-path workspace reduced to 1,637-1,641 relevant attempts after exclusions and stayed within every aggregate budget. Remaining partial statuses come from explicit topology, exclusions, monorepo, encoding or containment gaps rather than hidden budget exhaustion. `policy_limits_calibrated=false` means these enforced ceilings are not a universal performance SLA.

### Restart Checkpoint And Commit Boundary

The private checkpoint is revision-, policy- and root-bound, run-salted and capped at 64 MiB under checkpoint/storage policy 1.1. Canonical state remains 2 MiB, HTML 5 MiB and the full output root 256 MiB/128 entries; current plus pending checkpoint files are bounded at 128 MiB. The checkpoint preserves cumulative counters and the private observed-file ledger, but not source excerpts, excluded sensitive path names or the raw graph. Fresh-process restoration validates compatibility and deliberately reports transient topology as incomplete. A synthetic 100,000-record checkpoint exceeded the former 2 MiB slot, fit the 64 MiB slot and restored all records/counters in a fresh Python process. CP-07 commits scan and context/Brief through versioned request schemas, validates selected references against this private ledger, then deletes it. Immediate identical replay is idempotent; changed replay, stale revision and old checkpoint/storage policy fail closed. This is logical recovery across state and sidecar files, not a claim of one filesystem transaction spanning both files or persisted crash injection.

### Topology-Guided Context Selection

After safe reads, CP-08 builds a transient evidence-backed directed topology: project/service roots, manifests, internal packages, API/storage/deployment/test surfaces and explicit relations such as `contains`, `depends_on`, `exposes`, `persists_to`, `deploys_as` and `tested_by`. It uses deterministic weak/strong components, cycle condensation, topological generations and bounded ancestor/descendant traversal as secondary ranking signals. Direct goal matches rank first; implementation/configuration precede documentation/tests at equal relevance; automatic whole-file selection reserves at least 4 KiB of model capacity per selected source and transient delivery shares the envelope fairly. The larger policy path ceilings remain available for explicit bounded line ranges. This is standard-library local processing, not GraphRAG, a graph database, embeddings or a shipped NetworkX dependency.
