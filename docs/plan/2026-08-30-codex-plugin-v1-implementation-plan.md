...
- Mode contract: `standard` is the default when the user requests a scan without naming a mode; `quick` is an explicitly requested fast overview; `deep` is an explicitly selected bounded continuation after a visible material gap. Classification never selects/escalates a mode. `quick -> standard -> deep` preserves the same run/root/evidence and uses cumulative file/byte/active-time counters; user/Codex waiting time is excluded. Owner-accepted v1.1 totals are quick 250 files/32 MiB/60 s, standard 2,000/256 MiB/480 s and deep 10,000/2 GiB/1,800 s. Mode-specific topology ceilings are 50,000/depth 20/20 s, 250,000/depth 40/90 s and 1,000,000/depth 50/300 s; per-file ceilings are 1/2/4 MiB. Targeted context is quick 16 paths/1 MiB/24 KiB model-facing, standard 64/8 MiB/48 KiB and deep 256/64 MiB/64 KiB, additionally bounded by the selected mode's remaining cumulative budget. Warn at 80%; checkpoint `deep` after at most each 2,000 files or 256 MiB. Exhaustion produces a gap, not an automatic retry/reset.
