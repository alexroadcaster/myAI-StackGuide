...
- Acceptance criteria: Predeclare Recall@k/nDCG@k, wrong-exclusion and hard-constraint checks, total context bytes/token measurement method, latency/memory methodology and thresholds before running. Keep initial 60/12/48-KiB caps explicit. Preserve 16/20 rubric and zero critical failures; calibrate authorized reads/handoff instead of blanket coding refusal. Use the actual 2,500-card CAT-10/CP-06 snapshot for capacity and relevance, retain a separately labeled 10,000-row synthetic headroom fixture, and stratify held-out judgments across the 14 containers, thin/dense leaves, baseline/expansion cohorts, aliases and secondary-category dedupe. No vectors/model provider dependency for lexical eval.
...
- Acceptance criteria: Record Brief/source/card-2.0.0/index-format-2/policy-2.0.0 versions and hashes, source_mode=catalog_only, retrieval_engine=sqlite_fts5, numeric result IDs, matched v2 fields, counts/bytes, exclusions and integration handoff. Prove an alias lookup and a CAT-07A catalog-description lookup through the actual packaged route. Verify corrections/resume, no-match versus index failure, mixed-v1/v2 incompatibility, activity unknowns, sparse cards, no network/install/private-index writes, offline renderer and state recovery. Measure build/index size and cold/warm behavior on the actual 2,500-card snapshot; use 10,000 synthetic rows only for headroom. Synthetic duplicates cannot prove recall or genuine catalog growth; runtime pass cannot prove model quality.
