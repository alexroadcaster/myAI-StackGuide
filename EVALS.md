...
CP-03.CAT-10 is `catalog_snapshot_frozen_cp06_handoff_ready`: the source-owned snapshot has 2,500 unique repositories, 126 taxonomy nodes and 2,630 placements, with exact source/schema/taxonomy/field-contract/output pins in its final report. It preserves CAT-09 `browser_performance_verified` evidence for the same HTML projection; three review records moved to evidence-supported existing leaves and 12 remain explicit review records. The active C9 tuple is `RepositoryCardV2`, ActivityV2, retrieval policy 2.1.0 and index format 3. CP-09 is `implemented_verified_local`: 23/23 retrieval/matching, 18/18 index/intake, 48/48 CP-03 and 33/33 C8 cases pass; both scorer CLI gates and builder parity pass for 2,500 rows/cards, 2,630 classifications, 126 routes/162 members and immutable-write rejection. CP-07 supplies an owner-accepted bounded writer/intake/preflight interface and CP-08 completed-local scanner/context evidence under their stated ceilings. Earlier index-format-2 CP-03/C8 and scorer results remain historical. The current scorer remains `synthetic_compatibility_only`, `promotion_ready=false` and `quality_thresholds_calibrated=false`: mixed primary-before-reference behavior is a non-blocking gap; held-out relevance, RU/EN evaluation, browser/CP-11, human usefulness, visual/privacy acceptance, live freshness, install and release remain out of scope.
...
| `EVAL-SCAN-MODES-01` (`verified_local_v13`) | Representative roots receive fresh explicit `quick`, fresh default `standard`, then a separate gap-backed confirmed `standard -> deep` session under policy v1.3; checkpoint capacity is exercised at 100,000 records. | Hidden escalation, counter reset, failure to warn/checkpoint or a bounded profile presented as exhaustive. | Enforce exact 2,000/30,000/100,000-file and 256 MiB/2 GiB/8 GiB totals, warn at 80%, checkpoint deep at most every 2,000 files or 256 MiB, preserve cumulative predecessor/deep counters, and report mode/classification separately. |
| `EVAL-TOPOLOGY-CONTEXT-01` (`verified_local_structural_and_task_specific`) | Synthetic topology cases plus a goal-specific run on the 139,618-path root cover connected/disconnected/cyclic dependencies and relevant implementation/configuration/documentation/test surfaces. | Graph heuristics fabricate architecture, choose central but irrelevant files or bypass the selected mode's topology/context envelope. | Use evidence-backed edges, condense cycles before DAG ordering, rank direct goal matches before related paths, distribute transient model bytes across sources, respect quick 50k/depth-20, standard 250k/depth-40 and deep 1m/depth-50 topology ceilings, apply targeted-context envelopes 16/1 MiB/24 KiB, 64/8 MiB/48 KiB and 256/64 MiB/64 KiB, and label partial-graph conclusions as inferences/gaps. |

CP-08 deterministic evidence covers the producer and CP-07 writer join: explicit/default modes, cumulative counters, topology-time/depth continuation, SCC condensation/generations, stable direct-goal selection, fair transient model-byte allocation, minimized output, deep confirmation, scan/context commits, fail-closed selection/budget/provenance joins, versioned Brief composition, correction invalidation and fresh-process checkpoint restore. Policy v1.3 retains the accepted aggregate limits, adds generated/runtime/cache/worktree exclusions observed on the fourth real root and increases only the private checkpoint slot to 64 MiB within the unchanged 256 MiB output-root envelope. The 139,618-path root reduced to 1,637-1,641 relevant attempts after exclusions; quick/standard/deep stayed within budget. A synthetic 100,000-record checkpoint restored in a fresh process, and the repaired task-specific run selected 12/12 direct goal matches across configuration, implementation and migrations. `EVAL-SCAN-MODES-01` is `verified_local_v13`; `EVAL-TOPOLOGY-CONTEXT-01` is `verified_local_structural_and_task_specific`. Persisted crash injection, joined recommendation behavior and human product acceptance remain CP-11/15 evidence; `policy_limits_calibrated=false` means no universal performance SLA is claimed.
...
CP-04 first revalidates its captured-result protocol against one paired `RepositoryCardV2`/policy-2.1.0/index-format-3 fixture set, then freezes a simple lexical/filter baseline over those same cards, held-out relevance judgments and query set before comparing the selected FTS5/BM25 implementation. No embedding, vector store or model/provider call is necessary to measure retrieval. A later semantic extension must demonstrate gain over this baseline and account for installation, latency, memory and maintenance costs; it is not a current gate.

Report Recall@k and nDCG@k over numeric `github_repository_id`; define relevance grades and multiple valid solutions. Report hard-constraint violations and false exclusions separately. Include RU/EN and technology aliases, current names, historical full-name aliases, separate upstream/catalog description matches, task language, replacement intent, sparse metadata, no-match, activity/observation distinctions and index failure. Bind actual captures to the CAT-10/CP-06 2,500-card v2 snapshot. Stratify held-out judgments across all 14 navigation-container domains, representative thin and dense leaves, baseline and CAT-07A expansion cohorts, aliases/renames, review-queue behavior and typed secondary-assignment dedupe. Do not use a random corpus dominated by the largest expansion leaves. Measure dedupe/diversity and useful candidates surviving the final evidence-pack budget, not just raw top-k.

Owner-accepted policy 2.1.0 capacity ceilings are a request-selected maximum of 150 fetched hits across query variants, 12 detailed cards and 160 KiB UTF-8 for the full evidence pack, with 200 KiB total controlled input. The frozen 2,500-card snapshot motivated the capacity correction; retrieval quality and latency remain unmeasured. CP-03 specifies the separate Brief/context allocation. The accepted FTS v2 starting weights are `full_name=5`, `full_name_aliases=5`, `upstream_description=3`, `catalog_description=3`, `topics=3`, `category_labels=1`, `use_cases=3`, `integration_surface=3` and `best_for=2`; they are uncalibrated. CP-04 freezes the v2 policy hash, thresholds and measurement before quality runs; tuning uses a development set and held-out data remains separate. Report actual bytes and tokenizer/method when token counts are measured; do not equate bytes/characters with exact tokens. No quality threshold or latency SLA is fabricated in this plan.
...
CP-03 historically supplied C1-C6/C9 v1 and synthetic linked examples in
`tests/fixtures/plugin_contracts.json`; these are not retrieval-quality evals. The
active C9 surface now requires an atomic v2 migration and new compatible captures;
the other accepted CP-03 contracts are unchanged.
The [C8 runner contract](evals/plugin-v1/runner-contract.md), scenario/result schemas and scorer are present. The active policy-2.1.0/index-format-3 fixture join covers allowed, no-match, denied and missing-index outcomes. Project-local current-run evidence passes 48/48 CP-03, 33/33 C8 and both scorer CLI gates. CP-09 separately covers the actual read-only retrieval/matching boundary; the scorer still proves synthetic compatibility rather than recommendation quality.
...
C9 policy 2.1.0 fixes a request-selected maximum of 150 total fetched hits including duplicates, 12 cards and 160 KiB evidence,
with a 200-KiB sum for controlled Brief/targeted-context/evidence/request
inputs. Host instructions, prior conversation and output are additional. All
capacity ceilings are owner-accepted, while relevance, token, latency and usefulness remain uncalibrated. The scenario/result formats and scorer historically passed
compatibility validation against corresponding schemas/examples and independent
...
- Eval scorer: current `implemented_verified` synthetic captured-result compatibility; CP-04 owns C8 and `evals/plugin-v1/evaluate_retrieval.py`. It consumes captured C9 results without executing retrieval/models and currently passes through the pinned project-local development environment. The actual lexical baseline, held-out judgments, quality thresholds and human calibration remain unimplemented; this is not a product-quality runner or verdict.
...
Current CP-05-C evidence is `measured_local`: all 19 local cases ran in separate blind `codex exec` roots, required named children/skills were observed, and `TB-012` correctly used no project dispatch. A separate read-only reviewer marked all routes, skills, outcomes, action boundaries and named checks `PASS`. The strict observed packet at `evals/agents/results/cp05c-2026-09-01/results.json` grades 19/19 with no failures, packet-local trace hashes verified and its immutable machine status remains `needs_owner_acceptance`; the owner separately accepted that packet on 2026-09-01. Neither the packet nor that acceptance proves omitted-action absence, runtime activation, browser behavior, recommendation usefulness or release readiness.

The owner accepted this packet on 2026-09-01. CP-05-C is therefore closed at `measured_local`; the grader's machine status remains an accurate pre-owner-acceptance protocol result and is not rewritten into `promotion_ready`.
