...
## 2026-09-01 - RepositoryCardV2 owner decision and CP-06 plan migration

- The owner accepted `RepositoryCardV2` as the only active public-card representation. This is a control-plane/contract decision, not schema, index or runtime implementation. CAT-10 remains the exact frozen source; its counts, hashes and prior CAT-08/09/10 evidence were not changed.
- Added the `CP-03-C9-V2` gate before CP-06. The recorded CP-03 46-check and CP-04 27-check/CLI v1 results remain historical evidence; the active card/activity/query/result/evidence-pack/manifest schemas, retrieval policy and four C8 captures must migrate and pass together before CP-06 generates assets.
- Registered one required card shape with `identity`, `catalog`, `descriptions`, `classifications`, `repository`, `delivery`, `activity`, `advisory`, `provenance`, `evidence` and `corpus_kind`. Numeric GitHub ID is canonical; catalog IDs are lineage; historical full-name aliases and upstream/catalog descriptions remain distinct; unknowns are explicit null/empty values; public card facts cannot live only in a sidecar.
- Registered FTS v2 as a derived projection with explicit current-name, historical-alias, upstream-description, catalog-description, topics, category-labels, use-cases, integration-surface and best-for columns. Initial weights are accepted but uncalibrated. The paired target is card 2.0.0, policy 2.0.0 and index format 2; mixed v1/v2 artifacts fail explicitly with no active fallback or rebuild.
- Updated active requirement, plan, test/eval, PRD, roadmap and team-contract wording for CP-03/04/06/09/11/15/16. Removed `data/plugin_catalog_metadata.json` from CP-06 public output scope. Remote CP-12-14, vectors, runtime permissions and UI design remain unchanged.
- Verification: `python -B -m unittest tests.test_codex_contracts -v` passed 13/13; `git diff --check` exited zero with line-ending warnings only; the corrected dependency-free marker check confirmed V2 markers in all eight active documents, all five downstream task blocks, the sidecar disposition and aligned FTS weights. A focused stale scan found the detailed plan's top lifecycle line still describing v1 as the current contract; that active header was corrected while its dated completion reports remained intact. The first ad hoc marker command was invalid because PowerShell consumed Markdown backticks; its safe regex replacement passed and no product artifact was affected.
- Independent read-only review found one P2 wording issue: two planning lines called the v2 set active while checked-in schemas/policy are still v1. Both now say required target set after `CP-03-C9-V2` acceptance and explicitly preserve the current v1 implementation state until that gate passes. No other findings remained; the review confirmed field/weight alignment, historical v1 evidence preservation, downstream ordering and no UI/remote/runtime expansion.
- Evidence ceiling is `owner_accepted_control_plane_updated`. No v2 schema/fixture, public cards, SQLite index, retrieval route, relevance, runtime, release or publication is implemented or verified by this planning change. Rollback reverses only this V2 planning entry and the corresponding active-document deltas; preserve all historical evidence and the frozen CAT-10 source.

## 2026-09-01 - CP-03-C9-V2 mandatory contract gateway

- Implemented the owner-accepted `RepositoryCardV2` contract as the only active public-card shape. Card `2.0.0` now has the required identity/catalog/descriptions/classifications/repository/delivery/activity/advisory/provenance/evidence/corpus sections; ActivityV2 preserves creation/update/push/verified commit/release/sync/observation separately. Candidate eligibility, query, result, evidence pack and index manifest use positive numeric `github_repository_id` and the paired card/activity/policy `2.0.0`, index format `2` tuple.
- Retrieval policy `2.0.0` contains exactly nine explicit FTS fields with the accepted uncalibrated weights: current name 5, historical aliases 5, upstream description 3, catalog description 3, topics 3, category labels 1, use cases 3, integration surface 3 and best-for 2. Numeric GitHub ID owns dedupe/tie joins; mixed v1/v2 pins, generic IDs, implicit rebuild and full-catalog fallback fail closed.
- Migrated all linked fixtures, C8 scenario/result schemas, scorer and semantic oracles to four v2 synthetic outcomes. Downstream recommendation memo and integration plan also moved to V2 numeric joins (`github_repository_id` / `selected_github_repository_ids`); catalog record IDs and merged IDs now remain lineage inside card identity only. Historical non-C9 v1 schemas remain locally resolvable but are not an active C9 fallback.
- Corrected the taxonomy contract's stale source pin to the exact CAT-10 manifest SHA-256 `d2acb067017707bf6a01fcdfcedf1cc5324719acc7648b449980a5d4cecb371e` and snapshot date `2026-09-01`; taxonomy content/categories did not change. Contract/case hashes, query hashes and serialized evidence measurements were recalculated only after the final object graph was stable.
- Used `design-catalog-contracts`, `design-recommendation-evals` and `maintain-control-plane`. Two bounded read-only agents audited the schema/source and eval surfaces; a final independent reviewer found the memo/plan string-lineage join as P1. The primary writer migrated that downstream identity surface and the same reviewer confirmed the P1 resolved with no remaining P1/P2. No agent wrote files; no UI design, CP-06 asset, source manifest, generated HTML, runtime, provider, network, Git history or external system changed.
- Verification: the semantic/metric subset passed 43/43. With the explicit disposable development validator path, `python -B -m unittest tests.test_plugin_contracts tests.test_plugin_retrieval_eval` passed 74/74, comprising 46 CP-03 and 28 C8 tests. The cases-only scorer returned four valid cases with `promotion_ready=false`; the captured scorer returned `retrieval_score_v2`, four compatible records, `passed=true`, `promotion_ready=false`, `quality_thresholds_calibrated=false` and `verdict=synthetic_compatibility_only`. `python -B -m unittest tests.test_codex_contracts -v` passed 13/13; 38 JSON files parsed; the focused stale C9 v1/generic-ID scan returned no matches; `git diff --check` exited zero with line-ending warnings only. Default sandboxed Python still lacks `jsonschema`; missing validation remains exit 2 rather than a skip or pass, and no dependency was installed or persisted in this run.
- Evidence ceiling is `v2_synthetic_contract_compatibility_verified`. This gateway does not prove lossless generation of 2,500 cards, SQLite index bytes/integrity, actual FTS5 routing, relevance, latency, browser/UI, human RU/EN usefulness, runtime activation, release readiness or publication. CP-06 is now unblocked and is the next implementation task; it must consume only the exact CAT-10 freeze and produce the 2,500 cards plus paired immutable index evidence.
- Rollback reverses only this uncommitted C9 V2 schema/policy/fixture/scorer/test/control-plane delta after checking later edits. Preserve CAT-10 source and reports, all historical v1 evidence, unrelated dirty work and the disposable validator environment; do not reset Git or delete shared artifacts.

## 2026-09-01 - CP-06 RepositoryCardV2 and SQLite FTS5 bundle

- Implemented CP-06 from the exact CAT-10 freeze with `evolve-catalog-pipeline`, `verify-generated-parity` and `maintain-control-plane`. Two bounded read-only agents independently audited the source-to-card mapping and SQLite/test surface before implementation; a third read-only reviewer repeated the final checks and returned `accept` with no P0/P1/P2. Only the primary agent wrote shared schemas, builders, generated assets, tests and control-plane files. No UI design, runtime dispatch, provider/network action, source refresh, Git history or external publication changed.
- Reconciled the post-freeze contract header without changing taxonomy semantics: the CAT-10 report now pins taxonomy SHA-256 `09dcaac99e1e1d7110e9ab64ef33e20654be225fb6ea81dc5bfbf3483f3a721f`. The v2 card schema accepts exact frozen lineage ID families, `review_bucket` classifications, up to 80 lossless language entries and the source-owned URI-reference form. The new regression increases the current CP-03 suite from 46 to 47 tests; the original 46-test gateway remains dated evidence.
- Added source-first builders `scripts/build_plugin_catalog.py` and `scripts/build_plugin_search_index.py`, the intentionally empty snapshot-bound `data/plugin_advisory_seed.json`, focused catalog/index tests and the generated public bundle under `plugins/myai-stackguide/assets/`. Generation rejects source/schema/taxonomy/field-contract drift before output. No `data/plugin_catalog_metadata.json` or fabricated advisory enrichment is produced.
- The card output contains exactly 2,500 unique positive numeric GitHub identities, 1,624 baseline and 876 expansion members, 55 historical aliases, 2,630 typed direct classifications, 12 review records and zero rejected cards. Every card uses the one required v2 section set and keeps catalog lineage, upstream/catalog descriptions, status/evidence/advisory meanings and creation/update/push/verified commit/release/sync/observation times distinct. `catalog.snapshot.json` is 13,943,388 bytes; card-set SHA-256 is `fceeaa7eaf1d83e280ed4244fed2717a820d59fcdc5b1aa849fd82f245f2ef5b`; maximum serialized card size is 7,515 bytes under the 24-KiB gate.
- The derived external-content FTS5 index has exactly 2,500 logical rows and nine explicit columns: current name, historical aliases, upstream description, catalog description, topics, category labels, use cases, integration surface and best-for. It records 2,630 classifications, uses SQLite 3.50.4 with FTS5, opens through `mode=ro&immutable=1`, rejects writes and leaves no WAL/journal sidecars. `catalog.search.sqlite` is 2,023,424 bytes; index SHA-256 is `e7ab7d42b8ac9fa9e348549ef108264309d8cc763cc23c4eeeffe07a650af5ff`; logical-row SHA-256 is `82e461aa9fb8568b9d7194ee0e90ce396c45d11e00d10dfaaff40f43b8014e52`; copied policy SHA-256 is `c1a150b36a10c67c0220df31af88f90d59e353bdca2f52cb469e06ac6bb7fdca`.
- Verification: `python -B scripts/build_plugin_catalog.py --check` and `python -B scripts/build_plugin_search_index.py --check` passed exact parity; `python -B -m unittest tests.test_plugin_catalog tests.test_plugin_search_index -v` passed 19/19; disposable Draft 2020-12 validation accepted 2,500/2,500 cards and the index manifest; the full post-build CP-03/C8 suites passed 47/47 and 28/28; both scorer CLI gates retained `promotion_ready=false`, `quality_thresholds_calibrated=false` and `synthetic_compatibility_only`; `python -B scripts/build_catalog_html.py --check` preserved the existing 3,931,957-byte HTML and SHA-256 `da6c85bfd0158123cd6a1f756f1b54dc6f277dce849ac4e5d29f8709319792f0`. The final reviewer independently repeated both builder checks, 19/19 tests, SQLite probes and `git diff --check`.
- Implementation corrections were bounded and retained in evidence: the first index build exposed a 20-versus-21 SQL placeholder mismatch; in-memory mapping exposed legacy `NOT_SET` values and one bare upstream homepage; the source audit exposed stale lineage/review-bucket/language-limit assumptions. These were corrected in the owning builder/schema, not hidden or coerced. Default Python still lacks `jsonschema`; standards checks used a workspace-local disposable target. One initial temp target inherited inaccessible sandbox ACLs, so validation moved to a fresh bounded target; both temporary validator directories were removed afterward. A direct PowerShell read of the generated manifest remained ACL-restricted for the final reviewer, while Python builders/tests read it successfully; clean-install permission evidence remains CP-16 scope.
- Evidence ceiling is `implemented_verified_build_package`. This proves deterministic source-to-card/index generation, version/hash pairing, SQLite build-time integrity, immutable read-only open and representative alias/catalog-description smoke only. It does not prove CP-09 runtime query compilation/routing or relevance, CP-10 UI, CP-11 integration usefulness, CP-15 human/privacy acceptance, CP-16 install/upgrade/release, live freshness or publication.
- Rollback removes only the CP-06 builders, advisory seed, generated plugin assets, focused tests, CP-06 schema/freeze reconciliation and corresponding active-document additions after checking later edits. Preserve the CAT-10 source/HTML/history, CP-03-C9-V2 contract, unrelated work and Git history; do not reset, delete shared evidence or silently enable a v1/whole-catalog fallback. The immediate gate is CP-05-C fresh-session acceptance; the next implementation owner is CP-07 local intake/state/runtime preflight, and CP-09 later consumes this exact paired bundle.

## 2026-09-01 - CP-05-C fresh-session behavioral evidence

- Loaded the CP-05-C workflow with `maintain-control-plane`, `design-recommendation-evals` and current official Codex agent/CLI documentation. The live CLI rejected six legacy custom-agent metadata keys (`ownership`, `inputs`, `outputs`, `stop_conditions`, `model_policy`, `handoff_requirements`) as unknown. Removed those unsupported top-level keys from all nine role TOMLs while preserving their semantics inside `developer_instructions`, model, effort and sandbox values. Added a focused test that rejects any future unsupported runtime field.
- Ran all 19 local readiness cases in separate blind `codex exec` roots. Evaluated roots could not read case oracles, prior packets or memory; private-project cases remained synthetic. Every required named role/skill was observed, and `TB-012` correctly used no project skill or delegation. Three initial routing ambiguities were treated as failures and repaired through bounded role-description clarification: test/model evidence versus independent review, domain invalidation versus permission enforcement, and missing-runtime-contract stop ownership. Only final blind reruns enter the pass packet.
- The two implementation cases used disposable exact-owned fixtures. Independent RED evidence was an assertion mismatch for `TB-004` and the fixture `NotImplementedError` for `TB-010`; each agent edited one owned implementation file, preserved the test/acceptance, and reran one focused unit test to `1/1 OK`. The primary independently repeated both tests successfully, then removed only the four disposable fixture files. No provider, MCP, server, browser, external write, Git history or product runtime was activated.
- Persisted the sanitized observed trace, independent review and strict result packet under `evals/agents/results/cp05c-2026-09-01/`. The trace binds the case set SHA-256 `7b0aac11e7df51c44fc8a8d56d0194bfc898c91548befcfe85a5a584e448e6e5`, exact loaded role/skill/config hashes, root/child IDs, actions, outcomes and latency/token metrics. A fresh read-only reviewer inspected the complete sanitized projection and exact oracle/protocol, returning 19/19 `PASS` with `complete_trace_reviewed=true`.
- Verification: `python -B scripts/grade_agent_evals.py --cases evals\agents\team-behavior-cases.json --results evals\agents\results\cp05c-2026-09-01\results.json` exited `0` with `status=needs_owner_acceptance`, 19/19 passed, no failures and packet-local trace hashes verified. `python -B -m unittest discover -s tests -p test_codex_contracts.py -v` passed 13/13 after the runtime-schema/routing changes. The CLI still emits a non-fatal skills-context budget warning; every required skill remained discoverable and present in the persisted traces.
- Evidence ceiling is `measured_local / owner_acceptance_pending`. Sanitized trace review and packet hashes cannot authenticate omitted raw actions, prove recommendation quality, CP-09 routing/relevance, browser/UI behavior, runtime activation, install/release/publication, or replace owner acceptance. CP-07 remains blocked until the owner accepts or rejects this packet. Rollback removes only the CP-05-C packet and supported-schema/routing-description/test delta after checking later edits; preserve unrelated CP-06 work and Git history.

## 2026-09-01 - CP-05-C owner acceptance and CP-07 dispatch

- The owner explicitly accepted `CP-05-C` and directed CP-07 implementation with CP-04 completion in a disjoint parallel lane. CP-05 is closed at `measured_local`: 19/19 blind fresh-session cases, independent review and packet-local integrity remain the evidence; `promotion_ready=false` and all runtime/product/browser/release limits remain unchanged.
- CP-07 is active under the accepted local plugin architecture. CP-04 owns evaluation files and CP-07 owns local state/intake implementation; root control documents remain sequentially owned. CP-12-CP-14, Agents API/SDK, credentials, MCP, remote services, deployment and publication remain inactive.

## 2026-09-21 - CP-07 bounded remediation without test expansion

- Applied the owner's requested narrow remediation after the independent NO-GO review. No new test files, cases, broad suites, UI/design changes, catalog regeneration, provider/network work or Git operation were added.
- Removed the unsupported generic operation-key promise from lifecycle contracts. Runtime replay is now explicitly bounded: answer/correction event IDs permit only an identical immediate retry from expected revision to expected plus one, including the `ready` outcome; start/resume/finalize use their natural state/revision/history rules.
- Added an independent runtime SHA-256 trust anchor for the exact bundled search manifest (`a172f0378cb804ecd4bd10f3be69e522b6c3ee6955635498c6a060202647861d`) before its pins are trusted. A changed manifest now fails as `index_incompatible`; its declared hashes cannot self-authorize a substituted package.
- A skipped answer can no longer request early `ready`, and a ten-question run with no accepted answer remains truthfully cancelled. State validation now requires at least one `answered` record for `ready`. Pre-Brief answer correction uses the accepted 2,000-code-point limit; post-Brief correction events retain 1,000.
- Tightened claims instead of building more machinery: CP-07 uses the deterministic bounded question bank with early completion; integrated adaptive routing remains a CP-11 proof. Each CP-07 command performs one render attempt and returns an explicit `render_only` retry signal; CP-10/11 may orchestrate one retry against the unchanged target. Pending artifacts remain fail-closed evidence rather than an unimplemented automatic-recovery claim. Full semantic validation of CP-08/09/10-owned downstream objects remains an integration gate, not a CP-07 closure claim.
- This remediation unblocks CP-08/09/10 consumers of the stable CP-07 writer/preflight interface. Additional CP-07-only test expansion is explicitly not a prerequisite; the remaining adaptive/recovery/retry proof is consolidated into CP-11.
- Verification was deliberately limited to the existing focused command `C:\Python314\python.exe -B -m unittest tests.test_plugin_intake tests.test_plugin_state -v`, which passed 13/13, plus one 1/1 rerun of the existing immediate-answer-replay case after its final condition was tightened. No new test, broad regression, browser, catalog, retrieval-quality or model eval suite was run. CP-07 remains `implemented_not_verified` for product closure until the owned downstream joins and one CP-11 lifecycle prove adaptive selection, recovery and bounded publication retry.

## 2026-09-21 - CP-07 owner acceptance and downstream handoff

- The owner accepted the recommended CP-07 boundary. CP-07 is now `implemented` at the bounded local writer/intake/preflight ceiling; this is not promotion to retrieval, UI, joined lifecycle, human-usefulness, install, release or publication evidence.
- CP-08/09/10 are unblocked against the stable CP-07 interface. CP-08 bounded scanner/context is the next primary slice; CP-09 may proceed against the frozen CP-06 bundle and CP-10-A may start from accepted fixtures. Adaptive question selection, interruption recovery and one bounded render retry are consolidated into the complete CP-11 lifecycle instead of triggering more CP-07-only tests.
- Active REQUIREMENTS, PLAN, detailed plan, TEST, EVALS, PRD and roadmap statuses were aligned. Historical dated evidence remains unchanged. No runtime source, test case, catalog/index asset, UI design, provider/network state, Git history or external system changed in this acceptance update.

## 2026-09-21T13:45:16+03:00 - CP-08 scan modes and topology-guided context contract clarification

- The owner requested a deeper CP-08 description after reviewing the active scan policy and NetworkX documentation. Clarified that `quick`, `standard` and `deep` are read-budget profiles rather than repository classifications or automatic passes: `standard` is the unnamed default, `quick` is an explicit fast-overview request and `deep` is an explicit gap-backed continuation. Escalation retains one run/root/evidence lineage and cumulative file/byte/active-time counters; human/Codex waiting time is excluded, and no limit triggers an automatic retry or reset.
- Added the CP-08 transient topology stage after containment/exclusion and allowlisted parsing. It uses evidence-backed typed relations plus deterministic weak/strong components, cycle condensation, topological generations and bounded goal-seeded ancestor/descendant traversal. Results persist only through existing topology facts, basis-linked inferences, gaps and safe references. GraphRAG, embeddings, a graph database, a persisted raw graph and a shipped NetworkX dependency remain outside Local V1; the runtime stays standard-library-only.
- Clarified the nested targeted-context envelope: at most eight paths, 256 KiB source and 16 KiB compact UTF-8 model-facing content, further limited by remaining cumulative mode attempts/bytes/active time. The common 10,000-entry/depth-12 topology boundary remains unchanged in `deep`, so large repositories may stay partial. Added planned mode-escalation and graph-guided context cases to TEST/EVALS and required representative measurements before any calibrated-limit, performance, coverage or usefulness claim.
- Verification: `python -B -m unittest tests.test_codex_contracts -v` passed 13/13; `git diff --check` exited zero with line-ending warnings only. The historical repository-local `plugins/product-agent-os/scripts/validate_control_plane.py` path is absent, so that validator was not run and no pass is claimed.
- Evidence ceiling is `owner_requested_control_plane_clarification`. No scanner/context runtime, NetworkX install, schema/policy value change, CP-08 runtime-case execution, real-project measurement, privacy enforcement, graph-selection usefulness or CP-08 completion is claimed. Rollback reverses only this documentation/control-plane clarification and preserves CP-07 implementation, accepted CP-03 contracts, catalog/index assets, history and unrelated work.

## 2026-09-21T14:21:22+03:00 - CP-08 expanded scan-policy v1.1 ceilings accepted

- The owner accepted the expanded CP-08 values after a read-only stress review of 30 catalog repositories spanning focused integrations, libraries, applications, frameworks, platforms and large codebases. The catalog snapshot showed a roughly 35 MiB median across all nonzero-size records and much larger P75/P90 tails; repository metadata is a scale signal, not eligible text bytes or runtime calibration.
- Versioned the active scanner policy to `local-scan-v1.1`. Cumulative read ceilings are quick 250 files/32 MiB/60 s, standard 2,000/256 MiB/480 s and deep 10,000/2 GiB/1,800 s. Mode-specific topology ceilings are 50,000/depth 20/20 s, 250,000/depth 40/90 s and 1,000,000/depth 50/300 s; per-file ceilings are 1/2/4 MiB. The sanitized structured scan response remains 256 KiB.
- Expanded targeted context to quick 16 paths/1 MiB source/24 KiB model-facing, standard 64/8 MiB/48 KiB and deep 256/64 MiB/64 KiB. This is transient CP-08 Brief-composition input and is not automatically duplicated into the separately versioned CP-09/11 88 KiB recommendation-call allocation. All reads remain charged to the selected cumulative scan budget and retain containment/sensitive exclusions.
- Added an 80% budget warning and deep checkpoints after at most each 2,000 file attempts or 256 MiB. Expanded classification thresholds to compact <=2,000 eligible files/12 manifests and standard <=25,000/100 manifests/50 service roots; monorepo precedence remains unchanged. Updated owning schemas, linked positive fixtures, focused contract assertions, scanner/architecture/task documentation and planned TEST/EVAL cases.
- Verification: six changed JSON-compatible policy/schema/fixture files parsed successfully. The default sandboxed `python -B -m unittest tests.test_plugin_contracts -v` ran all semantic/workspace tests but correctly failed the schema class because `jsonschema` is not installed there; rerunning through the existing development-only validator path passed 48/48. `python -B -m unittest tests.test_codex_contracts -v` passed 13/13. No package was installed and no runtime scanner case was executed.
- Evidence ceiling remains `accepted_contract_not_runtime_calibrated`: `policy_limits_calibrated=false`. This change does not implement scanner/context runtime, prove filesystem containment, measure actual eligible-file coverage or latency, or claim a performance SLA. Rollback reverts only this v1.1 policy/schema/fixture/control-plane delta after checking later edits; preserve CP-07, catalog/index assets, history and unrelated work.

## 2026-09-21T15:11:26+03:00 - CP-08 bounded producer seam implemented and focused-test verified

- Implemented `plugins/myai-stackguide/scripts/scanner.py` and `context.py` with the project `build-stackguide-plugin`, `design-context-contracts` and `audit-readonly-boundaries` workflows. The runtime uses only the standard library: no NetworkX, embeddings, GraphRAG, graph database, install, network call, project import, build or script execution was added. CP-07 remains the only state writer.
- Added one compact `tests/test_plugin_scanner.py`; all synthetic projects use `TemporaryDirectory`, so no fixture tree or large generated corpus was created. Coverage includes exact policy/mode behavior, cumulative continuation, deterministic components/SCC condensation/generations, stable goal selection, transient excerpts, sensitive/no-execution/encoding cases, available symlink/hardlink checks, cumulative topology time, depth continuation, multi-manifest merge, schema bounds and hostile credential-shaped symbols.
- Preserved failure evidence instead of looping: the first evaluator pass ran 6 tests and exited 1 with one shared Windows containment defect plus one test-loader defect. The loader was corrected; one bounded runtime repair switched traversal link-count evidence to fresh `Path.lstat()` and typed the 64-bit `GetFinalPathNameByHandleW` call. Primary post-repair verification passed 6/6.
- Independent review initially returned NO-GO on four P1 findings. One bounded remediation made topology time cumulative with an 80% warning, retained depth-pruned directories for higher modes, merged evidence for semantically identical topology nodes, minimized credential-shaped manifest symbols and enforced the context-selection `brief_version` ceiling. The focused suite then passed 7/7. A final privacy-only bypass canary was added to the same regression method and passed 1/1; independent re-review returned GO with no P0/P1/P2 implementation findings at the producer-only ceiling.
- Final documentation/control-plane verification ran `python -B -m unittest tests.test_codex_contracts -v` once and passed 13/13. `git diff --check` exited 0 with non-fatal LF-to-CRLF warnings only. No broad catalog, retrieval, browser, model, provider or release suite was run.
- Evidence ceiling is `implemented_verified_producer_seam`. This does not implement or prove versioned Brief composition/recomposition, the missing CP-07 producer-to-writer adapter, saved scan/Brief state, phase transition, restart/replay, publication recovery, direct `quick -> deep` semantics, real-project coverage/performance/usefulness, calibrated limits, host-wide isolation, universal secret detection or universal filesystem-race resistance. `policy_limits_calibrated=false` remains unchanged.
- Rollback removes only the two new producer modules, the one focused test file and this active status/control-plane delta after checking later edits. Do not change existing state/history, schemas, catalog/index assets, permissions or Git history. The next sequential owner must define the CP-07/08 writer adapter before CP-11 can claim an integrated lifecycle.

## 2026-09-21 - CP-08 producer/writer join and standard real-project calibration

- The owner accepted recommendations 1-6 and selected two additional local calibration roots, later correcting the second to `codex_docs`. Work used the repository `design-context-contracts`, `build-stackguide-plugin`, `audit-readonly-boundaries` and `maintain-control-plane` workflows with bounded non-overlapping agent ownership. Existing dirty work and unrelated catalog/index assets were preserved.
- Implemented visible material-gap plus explicit `deep` selection for every deep continuation. Direct `quick -> deep` is valid without `standard` but additionally requires `explicit_confirmation=true`, preventing a shortcut inferred from the gap alone. Added compact revision/policy/root-bound private checkpoints, fresh-instance restore and replay validation. Checkpoints preserve cumulative counters and minimized evidence, omit raw graph/source/excluded sensitive path names, cap at 2 MiB and mark transient topology incomplete after restore.
- Added CP-07 `commit-scan` and `commit-context` adapters, bounded command payloads, immediate idempotent replay and fail-closed stale/changed replay. Versioned Brief composition retains references rather than excerpts. Post-Brief correction updates the canonical Brief domain and invalidates `selection`, `request`, `retrieval_result`, `evidence_pack` and `recommendation_memo`. State remains version `1.1.0`; restart data lives in a private sidecar rather than expanding canonical session state.
- Added the checkpoint and scan/context commit-request schemas and aligned the session workspace/team contracts. The physical state and sidecar files are revision-coupled and fail closed, but are not claimed as one cross-file filesystem transaction.
- Focused verification intentionally avoided a rerun loop. The first combined 17-method command passed 12 methods; three failures were stale expectations/fixtures for the new deep gate, synthetic pins and changed-replay setup. One bounded repair pass followed, then those three methods passed 3/3. The fresh-instance checkpoint/privacy method passed 1/1 after compacting duplicate checkpoint records. No final broad 17-method rerun was performed; evidence is complete only across the initial run and targeted reruns.
- Standard-mode calibration read the current catalog, `SOL-0.6` and `codex_docs` without network, project execution, dependency installation or persistent writes to the target projects. All three returned `partial` and `large_or_monorepo`, reached exactly 2,000 file attempts after about 15-46 MiB, restored in-memory checkpoints under 2 MiB and preserved counters. No standard budget remained for targeted context. Real scans exposed and repaired duplicate package-name identity and a topology-completeness contradiction.
- The sanitized report is `docs/reports/cp08-real-project-calibration-2026-09-21.json`. It retains aliases and aggregate counters only. The sample contains no compact/ordinary project and no quick/deep run, so it does not justify changing v1.1 limits or setting `policy_limits_calibrated=true`.
- Evidence ceiling is `partially_verified`: producer/writer/Brief/checkpoint behavior has focused local evidence and CP-08-C has standard-only real-project measurements. Explicit owner confirmation is still required before a new in-memory standard-to-deep run. Deep coverage, selected-context usefulness, persisted crash injection, human acceptance, complete CP-11 lifecycle, install, release and publication remain open.
- Final checks: runtime `py_compile` exited 0; `tests.test_codex_contracts` passed 13/13; `git diff --check` exited 0 with line-ending warnings. The CP-03 contract command completed 37 semantic/workspace methods but failed its schema-class setup because the disposable TEMP validator had been partially purged and no longer exported `Draft202012Validator`. No reinstall or retry loop followed. A fallback parsed 31 JSON schema/documents and confirmed all 171 local reference roots resolve; full Draft 2020-12 validation for the three new schemas remains an environment-blocked gap.
- Independent final review initially returned NO-GO with three P1s: `standard -> deep` lacked the separate confirmation flag, `commit-context` did not resolve selected paths/budgets against committed scan evidence, and Brief provenance IDs were not joined to current scan/intake state. One bounded remediation pass fixed only those surfaces. The first two-method remediation run exposed one stale positive call and an error-class mismatch; one small test/runtime repair followed. The exact two affected methods then passed 2/2 in 9.893 seconds. Fresh read-only re-review confirmed all three P1s closed and requested documentation alignment only; no further runtime/test cycle was required.

## 2026-09-21 - CP-08 confirmed deep calibration and scale remediation

- The owner explicitly confirmed one fresh read-only cumulative `standard -> deep` calibration cycle for the current catalog, `SOL-0.6` and `codex_docs`. The run used existing authorized local reads only: no network, dependency installation, project import/execution, target-project write, credential use, Git operation or external publication occurred.
- The first confirmed deep pass exposed three scale defects rather than being treated as product evidence: duplicate checkpoint candidate/record/read-path serialization exceeded the 2 MiB checkpoint envelope; independently minimized manifest and summary evidence broke the safe provenance join; and the scanner-summary/Brief size relationship could produce `brief_too_large`. One bounded remediation compacted checkpoint representation with backward-compatible restore, prioritized retained evidence into the manifest, joined report evidence to read manifest records and constrained the summary to a Brief-compatible 4 KiB envelope without allowing Brief observations to diverge.
- The first exact three-method regression command retained its failure evidence: a stale duplicated-`read_paths` expectation, a summary minimization ordering defect and an overly high dense-fact threshold failed. One small repair changed the assertion to the canonical read dispositions, preserved fact-referenced evidence while dropping unreferenced evidence first and adjusted the dense-fixture threshold. The same three methods then passed 3/3 in 16.430 seconds. No broad suite or repeated test loop followed.
- Final cumulative runs reached every final eligible file without exhausting `deep`: the current catalog reached 5,694 attempts and about 92 MiB in 28.0 seconds; `SOL-0.6` reached 2,740 attempts and about 114 MiB in 25.9 seconds; `codex_docs` reached 5,913 attempts and about 102 MiB in 38.0 seconds. Standard had stopped all three at exactly 2,000 attempts. Remaining `partial` reasons are explicit non-budget gaps such as exclusions, monorepo signals, encoding errors or unsupported containment; `deep` is not claimed exhaustive.
- Fresh ScannerSession restore accepted minimized checkpoints of 1,476,422, 656,456 and 1,347,513 bytes with continuous counters. Context composition selected 12, 14 and 8 fully scan-backed references, read about 109 KiB, 66 KiB and 700 KiB, and produced 6,183-, 6,036- and 4,914-byte Briefs. All three reached the 64 KiB transient model envelope and reported truncation rather than hiding it. Generic calibration terms matched no paths, so this is structural-source evidence only, not semantic or human usefulness evidence.
- Replaced the sanitized calibration report with version 1.1.0. It retains aliases, aggregate counters, typed reason codes, byte sizes, source buckets and continuation state only; absolute roots, selected paths, excerpts, source content, environment values and root fingerprints are absent. Active requirements, plan, scanner documentation, tests and eval status now describe `partially_verified_deep_three_projects` / structural-context evidence.
- Evidence ceiling remains `partially_verified`. The results support keeping the accepted v1.1 ceilings unchanged: `deep` had ample headroom for these three roots, while the 2,000-file `standard` profile intentionally yielded an actionable continuation gap. They do not cover quick mode, compact/ordinary projects, task-specific semantic relevance, persisted crash injection, human acceptance, a performance SLA, universal projects, install, release or publication. Therefore `policy_limits_calibrated=false`; CP-11 may consume the current seam, while broader calibration belongs to CP-15 or a separately assigned slice.
- Rollback reverts only this CP-08 scanner/context/test/report/control-plane delta after checking later edits. Preserve prior CP-07 state/history, CP-06 catalog/index assets, accepted schemas, unrelated dirty work and the two read-only calibration projects.

## 2026-09-21 - CP-08 policy v1.2 and three-mode real-root retest

- The owner replaced the aggregate scan ceilings with policy `1.2.0` / `local-scan-v1.2`: quick 2,000 files/256 MiB/120 seconds, standard 30,000/2 GiB/1,200 seconds and deep 100,000/8 GiB/4,000 seconds. Topology, per-file, targeted-context, classification, exclusion and no-execution rules remain unchanged. Deep checkpoint cadence remains each 2,000 file attempts or 256 MiB, and its independent serialized envelope remains 2 MiB.
- Separated the unchanged scan-report/domain schema version `1.1.0` from the new policy version `1.2.0` in scanner/context/intake runtime and linked schemas. Report, manifest, selection and checkpoint policy references now bind v1.2; old v1.1 sidecars fail closed by policy ID/version/hash rather than being silently resumed under larger budgets. Manifest/checkpoint counters now admit 100,000 attempts and 8 GiB.
- Before real-root reads, a bounded ten-check preflight passed nine methods. The 5,501-file checkpoint regression exposed one stale expectation: v1.2 standard now completes that fixture instead of producing the continuation gap. One test-only repair moved the setup to quick, preserving the intended lower-to-deep transition; the exact method passed 1/1 in 12.522 seconds. No broad preflight rerun followed.
- The progressive minimized report is `docs/reports/cp08-mode-recalibration-2026-09-21.md`. It was created before scanning and updated after every project. It retains aliases and aggregate counters only; absolute roots, selected paths, excerpts, source text, environment values and root fingerprints are absent.
- Each approved root received a fresh quick session, a fresh standard session and a separate confirmed cumulative standard-to-deep session. This provided comparable fresh mode observations while preserving the deep material-gap/confirmation contract. No network, dependency installation, project import/execution, persistent target-project write, Git operation or external publication occurred.
- Current catalog: quick stopped at 2,000 attempts/13,358,945 bytes/12.738 seconds; fresh standard reached 5,694 attempts/90,020,739 bytes/36.709 seconds without budget exhaustion; deep added one 2-4 MiB-eligible file and finished at 5,695 attempts/92,306,068 bytes/28.456 cumulative seconds. Its checkpoint was 1,476,461 bytes and context selected 12/12 backed references into a 6,183-byte Brief.
- `SOL-0.6`: quick stopped at 2,000 attempts/17,928,836 bytes/12.318 seconds; fresh standard reached 2,740 attempts/114,208,234 bytes/23.931 seconds without budget exhaustion; deep added no read work. Its checkpoint was 656,378 bytes and context selected 14/14 backed references into a 6,036-byte Brief.
- `codex_docs`: quick stopped at 2,000 attempts/46,146,842 bytes/22.086 seconds; fresh standard reached 5,911 attempts/94,320,839 bytes/20.092 seconds without budget exhaustion; deep added two larger-per-file reads and finished at 5,913 attempts/101,641,668 bytes/20.041 cumulative seconds. Its checkpoint was 1,347,446 bytes and context selected 8/8 backed references into a 4,914-byte Brief.
- Across all roots, quick emitted `files_80_percent` and stopped at its file cap. Standard reached the scanner-reported eligible set with substantial headroom. Deep added zero to two reads; non-budget exclusions, monorepo, encoding or containment gaps remained explicit rather than being presented as completeness. All context runs reached the unchanged 64 KiB model-facing envelope with explicit truncation and structural-only usefulness.
- Focused post-change evidence: the CP-07 scan/context commit integration method passed 1/1 in 10.216 seconds. The runtime report/policy split method passed. The added semantic schema-maxima assertion initially used the positive-fixture registry for the private checkpoint schema and errored with a missing key; one test-only repair loaded the exact schema directly, and the affected method passed 1/1. Control-plane contracts passed 13/13; seven JSON documents parsed; four Python files compiled from source; `git diff --check` exited zero with line-ending warnings only.
- Draft 2020-12 `SchemaContracts` did not run because the development-only `jsonschema` validator is unavailable in this environment. Its setup failure is retained as an environment-blocked gate, not a pass or skip; no package was installed and no retry loop followed.
- Independent final review found one P1 before closure: runtime checkpoints always include `session.limit_overrides`, but the strict checkpoint schema omitted it under `additionalProperties=false`. A bounded schema-only parity repair now requires the field, restricts mode/override names and caps each override at the corresponding v1.2 policy value. The existing checkpoint regression now compares runtime keys with the schema required/property sets; targeted verification and reviewer re-check follow this entry.
- The reviewer also found that the private checkpoint and two runtime commit-request schemas were absent from the mandatory positive registry, making a future available Draft validator deterministically RED, and that v1.1 sidecar rejection was only indirectly covered. The registry now materializes all three joined positives from canonical existing fixtures and expects all 26 schemas. The checkpoint regression submits an explicit v1.1-pinned sidecar and requires fail-closed rejection. These are contract/test-only repairs; real-root measurements remain unchanged.
- Targeted contract closure passed 2/2 in 6.588 seconds: the checkpoint runtime/schema parity plus explicit v1.1 rejection method, and the dependency-free 26-schema/policy/maxima registry method. A direct checkpoint schema required/property-key assertion passed; `git diff --check` remained zero with line-ending warnings only. Draft validation is still environment-blocked and is not promoted to a pass. Independent re-review is required before replacing the reviewer's NO-GO.
- Independent read-only re-review returned GO with no remaining P0/P1/P2. Both checkpoint P1s and the explicit v1.1 fail-closed P2 are closed at the static/targeted evidence ceiling. The unavailable Draft gate remains blocked and the overall status remains `partially_verified_v12_three_modes_three_projects`; no real-root rerun, release claim or policy calibration promotion follows.
- Evidence ceiling is `partially_verified_v12_three_modes_three_projects`. The three roots reach at most about 5.9% of the deep file ceiling, so this does not prove 30,000/100,000-file behavior, a 100,000-file checkpoint under 2 MiB, a performance SLA, universal coverage, semantic/human usefulness, persisted crash recovery, install, release or publication. `policy_limits_calibrated=false` remains correct.
- Rollback reverts only the v1.2 policy/schema/runtime/test/report/control-plane delta after checking later edits. Preserve historical v1.1 evidence, prior CP-07/08 state/history, CP-06 catalog/index assets, unrelated dirty work and the read-only calibration projects.

## 2026-09-21 - CP-08 policy v1.3 large-project closeout

- The owner supplied one additional read-only project containing 139,618 filesystem paths and requested full CP-08 closure. Work followed `design-context-contracts`, `build-stackguide-plugin`, `audit-readonly-boundaries` and `maintain-control-plane`. The target is retained only as alias `mypartners-large`; no absolute target root, source content, selected paths, environment values or root fingerprint is persisted in the closeout report.
- Host inventory exposed generated/runtime/cache/worktree trees that were not consistently excluded. Policy `1.3.0` / `local-scan-v1.3` retains the accepted quick 2,000/256 MiB/120 s, standard 30,000/2 GiB/1,200 s and deep 100,000/8 GiB/4,000 s ceilings while denying the observed generated segments before allowlist evaluation. The large root reduced to 1,637-1,641 relevant attempts and 33.7-39.1 MiB; fresh quick, fresh standard and confirmed cumulative deep exhausted no aggregate budget.
- Checkpoint/storage schema `1.1.0` raises only the private checkpoint slot to 64 MiB. Canonical state remains 2 MiB, HTML 5 MiB and the complete output root 256 MiB/128 entries; current plus pending checkpoint files are bounded at 128 MiB. Legacy state remains read-only and old checkpoint/storage writes fail closed. A synthetic 100,000-record checkpoint exceeded the old 2 MiB slot, fit the new slot and restored all 100,000 records/counters in a fresh Python process; the exact test passed 1/1 in 8.250 seconds.
- The first large-root context run exposed a real semantic defect: selection was capped by the 4 KiB public-summary evidence sample, topology-related paths preceded direct goal matches and a first large document could consume the model allowance. One bounded repair exposes the complete observed-path ledger only transiently, validates committed references against the private checkpoint under the CP-07 lock, ranks direct goal matches and implementation/configuration first and shares model bytes fairly. The repeated standard run selected 12/12 direct matches across configuration, README, Python/TypeScript implementation and migrations; it read 110,913 bytes and delivered 42,882 model-facing bytes with explicit truncation.
- The previously authorized development-only Draft validator was installed only under ignored `.codex-tmp/cp08-validator`; no global or plugin dependency changed. The first schema pass exposed four synchronized v1.3/storage fixture branches, one bounded repair aligned them, and the final full contract run passed 48/48. Final current-run verification also passed scanner/intake 18/18 and control-plane contracts 13/13. Earlier failed method-name selection, missing import and helper-argument mistakes are retained as command/test-harness evidence and were not treated as product failures.
- Final boundary hardening revalidates the persisted checkpoint against the current root/run/policy before `commit-context` and admits only `read` records whose evidence ID is deterministic for the path. The affected checkpoint/commit methods passed 3/3 in 25.066 seconds; the already clean broad runtime/contract suites were not repeated.
- The progressive sanitized evidence is `docs/reports/cp08-large-project-closeout-2026-09-21.md`. CP-08 is now `completed_local_v13`. Persisted crash injection, joined recommendation behavior, human product/privacy acceptance, installation and release remain owned by CP-11/15/16. `policy_limits_calibrated=false` remains intentional because this local evidence does not establish a universal performance SLA or exhaustive arbitrary 100,000-source-file calibration.
- Rollback reverts only the v1.3 scanner/context/storage/schema/test/report/control-plane delta after checking later edits. Preserve prior CP-07 state/history, CP-06 catalog/index assets, historical v1.1/v1.2 evidence, unrelated dirty work and all read-only calibration projects. No Git history or external publication operation was performed.

## 2026-09-22 - CP-09 retrieval capacity policy v2.1

- The owner accepted the recommended CP-09 capacity increase. Retrieval policy `2.1.0` / `fts5-local-v2.1` now permits at most 150 retrieved hits, retains 12 detailed cards, raises the serialized evidence pack to 163,840 UTF-8 bytes and raises the complete plugin input envelope to 204,800 bytes. A request may select a lower hit budget; none of these ceilings is a quality, latency or token-use target.
- Propagated the versioned contract through the owning retrieval/recommendation schemas, runtime package trust pins, generated read-only SQLite bundle, C8 fixtures/scorer and focused boundary tests. Updated active control-plane references so no current document continues to present the superseded 60-hit/48-KiB/88-KiB contract as active. Historical dated evidence was not rewritten.
- Regenerated the bundle reproducibly at `2026-09-21T18:20:47Z`. The 2,500-card SHA-256 remains `fceeaa7eaf1d83e280ed4244fed2717a820d59fcdc5b1aa849fd82f245f2ef5b`; policy SHA-256 is `ff6e8444c4664492bb099f0819b2d284aa5c28f0d44c7a745c63f434c3489dbf`; index SHA-256 is `acc1ddba2945c4ce2014178d61d923419f4d569a6ae77629790b8e7adbdab9c7`; manifest SHA-256 is `9cbb259aac4c75707814a3eb4f6811214146539d13a36f32aaae9e2ef072f8c8`.
- Current focused verification passed: package parity/read-only verification; 81/81 retrieval contract and C8 checks; 18/18 search-index/intake checks; 13/13 Codex control-plane checks; C8 CLI compatibility and quality-plan gates. Both evaluation gates retain `promotion_ready=false` and `quality_thresholds_calibrated=false`; the evidence ceiling is `synthetic_compatibility_only` for recommendation quality.
- `python tools/check_docs.py` was unavailable because that repository path does not exist. Full `git diff --check` could not hash regenerated bundle files after the Windows sandbox created them with a protected local ACL; the owner declined the proposed ACL normalization. Git status and non-bundle text diff remain readable, and product builders/tests read and verified the bundle. No permission change, package installation, network call, Git history operation or publication was completed.
- Rollback must treat policy source, dependent schemas/pins, C8 fixtures/tests and the three changed generated bundle artifacts as one compatibility unit. Preserve CP-08 work, catalog card content, historical evidence and unrelated files.

## 2026-09-22 - Control-plane context and verification budget

- Added explicit micro, contract-value and structural change sizing. Micro and contract-value work now reads only the owning source and direct consumers, uses at most one primary skill, does not spawn subagents and must justify expansion beyond eight files or two control-document families.
- Narrowed `maintain-control-plane` to material drift in active status, ownership, dependencies, acceptance or evidence. Its workflow now uses progressive disclosure instead of loading the full PRD/PLAN/TEST/EVALS/RUNLOG chain.
- Added schema-versus-policy revision rules and a bounded verification stop rule. Unrelated environment or ACL failures are recorded once and do not trigger repair or retry loops unless the requested outcome depends on them.
- Preserved the configured `gpt-5.6-sol` / `high` baseline because no current same-case `high` versus `medium` comparison exists. The policy now reserves high effort for structural or high-risk work and requires measured evidence before changing durable defaults.
- Corrected the active R11 retrieval-policy pin to `2.1.0`, replaced stale CP-05 current-work text with the accepted CP-09/10 sequence and labeled the older TEST policy-2.0 statement as historical. No product runtime, policy value, schema, generated bundle or Git history changed.
- Current focused verification passed `tests.test_codex_contracts` 13/13 and the scoped `git diff --check`. The external skill helper did not run because its environment lacks `PyYAML`; this was recorded once without installing a package or retrying, while the repository suite independently validated skill frontmatter and discovery metadata.
- Residual structural work is intentionally separate: the current CP-09 `2.1.0` artifacts still use the established combined policy-version pin, and there is no new generated current-status registry. Splitting that pin into schema version plus policy revision, or introducing a generated registry/helper, requires its own compatibility migration and representative checks; it was not mixed into this instruction-only correction.

## 2026-09-22 - CP-09 retrieval/runtime acceptance evidence reconciliation

- Current CP-09 acceptance is `accepted_structural_runtime` under active index format `3`: `python -B -m unittest tests.test_plugin_retrieval tests.test_plugin_matching -v` passed 23/23; `python -B -m unittest tests.test_plugin_search_index tests.test_plugin_intake -v` passed 18/18; and `python scripts/build_plugin_search_index.py --check` passed for 2,500 rows/cards, 2,630 classifications, 126 routes/162 members and immutable-write rejection. The acceptance matrix covers all 111 leaf, 14 container and one review route; secondary/container numeric-ID dedupe; valid-missing fail-closed behavior; RRF `k=60` allocation/redistribution/cap-150/ties; exact four matched fields; UTF-8/request-selected pack accounting; typed unavailable/corrupt/incompatible errors; and no writes, rebuilds or sidecars.
- Independent post-fix review confirmed rejection of a tampered supplied card, invalid UUID and forged complete manifest-pin tuple; normal production and synthetic packs are ready with no remaining S1 runtime finding. This is CP-09 structural/runtime evidence only. The overall ceiling remains `partially_verified`: mixed primary-before-reference interaction is a non-blocking gap, and relevance, RU/EN, browser, CP-11, human and release evidence are out of scope.
- Earlier index-format-2 CP-03/C8/scorer results remain dated history. In the current environment `SchemaContracts` and the scorer cannot run because `.codex-tmp/cp08-validator` is access-denied/unavailable; no historical 48/48 or scorer pass is represented as current-run evidence. No package installation, permission repair, source/test/schema change, Git operation or publication occurred.
- Rollback reverts only this `TEST.md`/`EVALS.md` active-claim reconciliation and this append-only run-log entry after checking later edits; preserve CP-09 implementation, index assets, historical evidence and unrelated dirty work. Next owner: CP-04/Quality Evaluator for relevance/scorer recovery and calibration; CP-11/CP-10 owners for joined browser/runtime and RU/EN evidence.

## 2026-09-22 - CP-09 local implementation closeout and permission correction

- The preceding claim that `.codex-tmp/cp08-validator` was access-denied was re-diagnosed. Direct read/write probes on its package files succeeded; the failing default Python process simply did not include that directory on `sys.path`. An explicit local-path run immediately passed `SchemaContracts` 11/11, proving the gate was dependency discovery rather than a CP-09 or schema defect.
- Added pinned development-only `jsonschema==4.26.0` and `referencing==0.37.0` in `requirements-dev.txt`, installed them into ignored project-local `.venv`, and changed the canonical contract/scorer commands to use `.venv\Scripts\python.exe`. The shipped plugin remains standard-library-only and no global Python package was changed.
- Fixed the real generated-file ACL defect in `build_plugin_search_index.py`: package bytes are now atomically published through a temporary file created in the destination `assets` directory, so manifest/index inherit the project ACL instead of retaining the protected DACL of `.codex-tmp`. Rebuilding with the original `built_at` retained the accepted index/manifest bytes and hashes. The manifest now inherits `CodexSandboxUsers`/project access, and full `git diff --check` passes.
- Current project-local verification passed: CP-03 plus CP-09 retrieval/matching 71/71, C8 33/33, search-index/intake 18/18, both scorer CLI gates, and `build_plugin_search_index.py --check` for 2,500 rows/cards, 2,630 classifications, 126 routes/162 members and immutable-write rejection. The scorer remains `synthetic_compatibility_only`, `promotion_ready=false` and `quality_thresholds_calibrated=false`; those are CP-04/15 quality limits, not unfinished CP-09 implementation.
- CP-09 is closed as `implemented_verified_local`. The mixed primary-before-reference interaction remains a non-blocking observed-data gap; relevance, RU/EN, browser/CP-11, human and release evidence remain downstream scope. No remote/MCP/vector/deployment activation or Git history operation occurred.

## 2026-09-23 - CP-10-A shell and offline language implementation

- Primary implemented the accepted A checkpoint without agents: `render_report.py`, `status-template.html`, paired `assets/locales/{ru,en}.json`, the sequential `state_store.py` renderer adapter and `tests/test_plugin_artifact.py`. Existing CP-07 validation, receipt, caps, lock/recheck and atomic replacement remain the sole publication boundary. Eight navigation targets expose honest source availability and explicitly unimplemented B-D details, saved revision/time/status, question counts and a Codex next action. No private state JSON or dynamic narrative is embedded; B owns those source-bound views. Static dictionaries are exact-key validated, text is escaped, and language/view switching uses only a validated fragment and embedded labels, without storage or network APIs. No-JS markup contains all eight views in the saved locale.
- RED: the start/publication test failed specifically because the former fixture had no `id="goal"`. GREEN: `.venv/Scripts/python.exe -B -m unittest discover -s tests -p test_plugin_artifact.py` passed 5/5 (both saved locales/no-JS structure, dictionary gaps/escaping, failure preservation/obsolete render, isolated CLI). `.venv/Scripts/python.exe -B -m unittest discover -s tests -p test_plugin_intake.py` passed 9/9. Extracted template JavaScript passed `node --check`; `git diff --check` passed. Python `-I` exposed a sibling-import error during development; the adapter now loads the exact packaged path, with a regression case.
- The additional existing `test_plugin_state.py` run is not green: after the import repair, four failed assertions remain in two test methods. Its lifecycle fixture still declares memo `2.0.0` while the current schema requires `2.1.0`; its correction assertion expects three invalidations while the accepted contract includes `selection` and `request` too. Those untouched legacy expectations were recorded, not silently changed or attributed to passing CP-10 checks.
- A synthetic start was published successfully under `.codex-tmp/cp10-a-preview/docs/myai-stackguide/status.html`. Browser verification could not start: the available in-app browser explicitly denied `file:` navigation by URL policy. No alternate browser/proxy/server bypass was attempted. Therefore CP-10-A is `partially_verified`, not browser-accepted; actual interaction, visual quality, focus and no-JS browser behavior remain pending. Node syntax and Python markup checks are not browser evidence. CP-10 B-D, CP-11 lifecycle, human/release acceptance remain open.
- Skills: build-stackguide-plugin governed the single-writer/offline boundary; python-code-style informed the Python implementation; maintain-control-plane limited status updates to this log and the owning task. The Playwright skill was inspected, but no CLI browser run or installation occurred. Rollback reverses only the six implementation/test files and these task/log edits, retaining all existing state/history/catalog assets. No permission, protected instruction, dependency, Git history, external publication or provider change occurred.

## 2026-09-23 - CP-10-B first four source-bound views

- Primary, without delegation, implemented Goal, Questions, Scan and Context in the existing `render_report.py`; added keyed RU/EN labels, narrative switch markup and readable detail styles in the paired dictionaries and `status-template.html`. The renderer consumes only committed state and the accepted scan policy, separates observed facts from inferences, shows question rationale and saved answer provenance, distinguishes scan requests from reads, and displays unknowns/partial coverage without inventing a Brief or recommendation. Missing translations retain the original with an explicit notice; consumed stale source digests fail the render before CP-07 publication. CP-07 remains the only writer/publisher.
- RED: two existing-fixture checks failed while all B panels were still placeholders. GREEN: `.venv/Scripts/python.exe -B -m unittest discover -s tests -p test_plugin_artifact.py` passed 7/7. `.venv/Scripts/python.exe -B -m unittest discover -s tests -p test_plugin_intake.py` passed 9/9. The extracted inline JavaScript passed `node --check`; `git diff --check` passed. A synthetic full-state fixture was published under `.codex-tmp/cp10-b-preview/docs/myai-stackguide/status.html`, and the isolated `retry-publication` CLI returned `publication_status=current` at saved revision 3. The initial preview-generation call used a relative project root and failed containment validation; resolving that root before the bounded rerun succeeded.
- Browser `file:` navigation was already denied by the available browser policy in A; no alternate browser/proxy/server route was attempted. Static markup/tests and CLI publication do not prove live language switching, keyboard focus, visual layout at 200%, print appearance or human usefulness. CP-10-B is `implemented_local, partially_verified`; C-D, CP-11, CP-15 and release remain open. Rollback reverses only the four CP-10 source assets, the focused artifact test additions and this status/log entry; preserve unrelated prior work and generated state/history.

## 2026-09-23 - CP-10-C Options, Compare and Integration

- Implemented the three remaining source-bound decision views in `render_report.py` with paired RU/EN interface labels and compact candidate, horizontally scrollable matrix and selectable handoff styles. Options shows every saved recommendation and otherwise unassigned captured card, request-specific eligibility without role promotion, public card identity/activity, actual retrieval/pack counts, pins, caveats, defer/revisit and reading path. Compare uses only saved structured cells, includes the no-change baseline only when specified, shows Brief constraints and retains unknown cells, counterargument and hard exclusions. Integration shows the proposed graph, prerequisite status, ordered steps/dependencies, not-run first validation, risks, rollback and scoped copyable handoff. Null memo/plan/diagram and absent translations remain explicit; no commands execute, no private state JSON is embedded and CP-07 remains the sole writer.
- One targeted test first failed against the C placeholders. Focused verification after implementation passed `.venv/Scripts/python.exe -B -m unittest discover -s tests -p test_plugin_artifact.py` 10/10, including RU/EN, blocked/unassigned cards and missing memo; `.venv/Scripts/python.exe -B -m unittest discover -s tests -p test_plugin_intake.py` 9/9; and `git diff --check` exited 0 with line-ending warnings only. No broad suite, model eval, browser workaround, provider, network, permission, external publication or Git history action was run.
- CP-10-C is `implemented_local, partially_verified`: fixture/static/CLI evidence does not establish retrieval relevance, visual quality, keyboard/200%/print behavior, human usefulness or joined CP-11 activation. CP-10-D owns History/recovery and full eight-view rendered acceptance. Rollback reverses only the C renderer, locale, template and focused artifact-test delta plus this log entry after checking later edits; preserve prior A/B work and generated session state.

## 2026-09-23 - CP-10 verification debt and R11 pin reconciliation

- Reproduced the four stale `test_plugin_state.py` assertions in two tests: three report-phase matrix cases still used memo schema `2.0.0`, and one post-Brief correction expected only three invalidations. Updated only those test expectations to the accepted memo `2.1.0` and full `selection`, `request`, `retrieval_result`, `evidence_pack`, `recommendation_memo` list. `.venv/Scripts/python.exe -B -m unittest tests.test_plugin_state -v` now passes 6/6; no runtime or schema behavior changed.
- Corrected active R11 in `REQUIREMENTS.md` from index format `2` to `3` and replaced the stale mixed-v1/v2 wording with mixed/stale active-pin rejection. The checked-in `catalog.search-manifest.json` pins index format `3` and retrieval policy `2.1.0`; historical format-2 evidence remains unchanged. `git diff --check` exits zero with line-ending warnings only.
- Browser acceptance remains open, not silently waived: the previous Browser Use URL-policy rejection explicitly forbids alternate browser/URL/CDP routes to achieve the same preview. The user's screenshot showed the source template, not a rendered session artifact. No new browser attempt, browser workaround or visual-acceptance claim occurred. CP-10-D/CP-15 still require policy-permitted rendered interaction and 200%/focus/print evidence or an explicit owner decision about the unmet gate. Rollback reverses only this test-expectation update, active R11 wording and this append-only entry after checking later edits; preserve the concurrent CP-10-C work.
