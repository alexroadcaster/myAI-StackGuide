...
CP-07's transient `runtime-preflight-result.schema.json` is the single preflight response; it is not persisted in `state.json`. Checks stop at the first failure in this order: input, CPython 3.14.x, selected-project root, actual SQLite FTS5 probe, package files, JSON/SQLite parse and integrity, then exact paired-v2 pins/hashes/flags. Blocked preflight writes no state and does not install, download, rebuild, use network, switch engines or dump the full catalog as fallback.
...
| --- | --- |
| `quick` reads | 2,000 files; 256 MiB text; 120 seconds total |
| `standard` reads | 30,000 files; 2 GiB text; 1,200 seconds total |
| `deep` reads | Up to 100,000 files; 8 GiB text; 4,000 seconds total, including prior work; no repeated reset of counters |
...
Topology and individual-file limits are mode-specific; escalation raises them only to the selected cumulative profile and never relaxes sensitive-source or containment rules. At 80% of a file/byte/time/topology ceiling, emit a warning in the current result. A `deep` continuation records progress, coverage and remaining gaps after at most each 2,000 additional file attempts or 256 MiB consumed; checkpoints do not reset counters or create extra allowance. CP-08 reports visited entries, eligible/attempted files, consumed bytes, active time, manifest/service-root counts, completion reason and task-specific selection evidence. Policy v1.3 retains the owner-accepted v1.2 aggregate ceilings and adds observed generated/runtime/cache/worktree exclusions. Four real roots include a 139,618-path workspace, and a synthetic 100,000-record checkpoint restores in a fresh process under the 64 MiB private slot. `policy_limits_calibrated=false` remains because this local evidence is not a universal arbitrary-project performance SLA; it does not keep CP-08 open.
...
The CP-03 addendum defines `specs/artifact/localized-presentation.schema.json`, `publication-result.schema.json` and affected intake/Brief/corrections/memo/plan/state joins; CP-07 adds the transient `specs/runtime/runtime-preflight-result.schema.json`. The [workspace contract](../artifact/session-workspace-contract.md) defines the 49-subsection source map, new-write version 1.1.0, read-only legacy 1.0.0 and semantic invariants. `html_revision` is a prior same-run observation at commit time, not the following publication result; transient publication/preflight outcomes never mutate finalized state. Presentation has a 512 KiB sub-limit within the unchanged 2 MiB state cap. Earlier CP-03/C8 static evidence does not cover these CP-07 changes; CP-04 calibration remains separate and Quality Evaluator must add targeted lifecycle/sanitizer/preflight cases. CP-07/10 still implement actual writing/rendering; no migration, renderer or translation runtime is delivered by these schemas.
