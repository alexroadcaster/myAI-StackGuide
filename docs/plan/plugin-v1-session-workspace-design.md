...
The existing C1-C6/C9 schemas already supply most decision data. Missing contract support must be explicit: the current state has no locale presentation contract, and some richer audience/workflow/integration interpretation needs a defined mapping rather than renderer-generated facts. CP-03 must register these fields before runtime accepts them. A UI proposal cannot silently extend strict schemas or claim CP-03 acceptance.
...
**Sources:** C1 intake, C3 Brief/summary, C6 revision. Audience/workflow or constraints not currently mapped by C3 need a CP-03 addition or remain explicitly unknown. **States:** no goal, idea/manual context, active, incomplete, finalized. **Primary action:** copy the next clarification for Codex.
...
2. **Narrative content.** Brief summaries, questions/rationales, fit/caveat explanations, comparison conclusions and integration/handoff prose need paired localized presentation fields tied to the same canonical domain field/evidence IDs and source revision. Produce them in the authorized host composition workflow, then sanitize/validate before saving. Never generate two independent recommendations. Original sanitized user statements and source quotations retain their language and attribution; a translation is a labeled additional view, not replacement source evidence. New translations of finalized historical content must not rewrite the immutable run; use an explicitly derived view/new run with source provenance under the accepted contract.
3. **Contract amendment.** CP-03 must specify `default_locale`, source language, presentation revision, translation coverage/status and field/locale mappings in a planned `specs/artifact/localized-presentation.schema.json`, joined to affected C1/C3/C5/C6 schemas. These are planned keys, not existing schema claims. Existing strict schemas and examples stay unchanged in this design task. Both languages must preserve negation, uncertainty, role, mandatory constraint, execution state and evidence references. Bind translated content to the relevant Brief version, memo/plan identity and content revision. A saved locale-preference-only change may advance the state revision without changing semantic content, invalidating retrieval or requiring a new translation; CP-03 must distinguish these revision domains explicitly.
...
| --- | --- | --- |
| CP-03 | Catalog Architect: map the eight views to contracts; add localized-presentation and missing structured context mappings, revision/invalidation/size semantics; update linked examples in its own assignment | Current schema contract plus R15 addendum accepted with C8/C9; twenty existing schemas are not retroactively promoted |
