...
**Current status:** the catalog and reproducible pipeline exist; agent/skill definitions and offline team checks are present. CP-01/02 documentation and [local ADRs](specs/decisions/plugin-v1-architecture.md) are amended to the owner's decisions. CP-03 includes the local schemas, [eight-view/presentation/publication contracts](specs/artifact/session-workspace-contract.md) and [linked examples](tests/fixtures/plugin_contracts.json). The mandatory `CP-03-C9-V2` gate makes `RepositoryCardV2`, ActivityV2, retrieval policy `2.1.0` and index format `2` the active paired contract. Policy 2.1.0 carries the owner-accepted CP-09 capacity ceilings while retrieval quality remains uncalibrated. CP-06 packages all 2,500 frozen repositories as schema-valid v2 cards plus a deterministic immutable SQLite FTS5 index, manifest and byte-identical policy. CP-07 supplies the bounded local writer/intake/preflight boundary. CP-08 is closed at `completed_local_v13`: policy v1.3 implements bounded read-only scanning, transient topology, task-aware references-only context, versioned Brief composition, corrections/invalidation, sole-writer commits and a 64 MiB private restart checkpoint. The [large-project closeout](docs/reports/cp08-large-project-closeout-2026-09-21.md) adds a 139,618-path project, a 100,000-record fresh-process checkpoint restore, task-specific context evidence and full local contract/runtime regressions. `policy_limits_calibrated=false` remains a truthful non-SLA flag, not an open CP-08 implementation gate. CP-04 retrieval-quality/human calibration and CP-09 onward runtime work remain open. The plugin is not installed or release-verified. Relevant authorized project context is allowed; [privacy boundaries](specs/decisions/plugin-v1-permissions.md#context-access-and-product-privacy) retain minimization and sensitive exclusions without a host-wide isolation promise.
...
The local scanner inspects an authorized project in read-only mode and builds a sanitized **Project Context Brief** containing:
...
| Read-only scanner | CP-08 is `completed_local_v13`: scan/context, CP-07 commit adapters, Brief corrections/invalidation and 64 MiB private restart checkpoints are verified locally. Policy v1.3 has all-mode observations on four real roots, task-specific selection on the 139,618-path root and a 100,000-record fresh-process checkpoint restore. Human product acceptance and release remain CP-15/16. |
| Local Codex plugin | Selected local direction; CP-06 assets and the joined CP-07/08 local state path exist, but CP-09/10/11 integration, installation and release remain unverified. |
...
3. Structured queries, weighted BM25 and RU/EN/technology aliases use a request-selected budget up to 150 fetched hits across all variants; constraints/dedupe produce at most 12 detailed cards and a 160-KiB evidence pack. Narrow requests may choose a lower candidate budget. These are owner-accepted engineering ceilings pending CP-04/11/15 evaluation, not measured relevance or performance.
